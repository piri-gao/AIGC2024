...
class EnvRunner(XSimEnv):
    ...
    def get_done(self, obs):
        ...
        if cur_time >= 20 * 60 - 1:
            ...
            done = self._print_score(done, obs, "1", red_area_score, blue_area_score)
        ...
        if red_done and not blue_done:
            ...
            blue_area_score = 0
            # red_round_score, blue_round_score = self._cal_score(obs)
            done = self._print_score(done, obs, "2")
            return done
        ...
        if blue_done and not red_done:
            ...
            blue_area_score = 0
            # red_round_score, blue_round_score = self._cal_score(obs)
            done = self._print_score(done, obs, "3")
            return done
        ...
        if red_done and blue_done:
            ...
            red_area_score = 0
            blue_area_score = 0
            done = self._print_score(done, obs, "4")
            return done
    ...
    def _print_score(self, done, obs, done_reason_code = "0", red_area_score = 0, blue_area_score = 0):
        filename = "logs/" + str(self.red_cls).split("'")[1] + "_VS_" + str(self.blue_cls).split("'")[1] + "_" + str(
            count) + ".txt"
        with open(filename, "a") as fileobject:
            # 输出结束原因
