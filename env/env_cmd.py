...
class CmdEnv(object):
    ...
    def make_linepatrolparam(receiver: int,
        ...
        for coord in coord_list:
            ...
            if owner_info["entity_name"] == "有人机":
                ...
                if coord.get("Z") < 2000:
                    coord["Z"] = 2000
                if coord.get("Z") > 15000:
                    coord["Z"] = 15000

            if owner_info["entity_name"] == "无人机":
                if coord.get("Z") < 2000:
                    coord["Z"] = 2000
                if coord.get("Z") > 10000:
                    coord["Z"] = 10000
    ...
    @staticmethod
    def make_jamparam(receiver: int) -> dict:
        '''
        干扰指令
        :param receiver: Handle ID
        :return: 完成组包后的数据，格式为dict：Key为主题名称，Value为数据
        '''
        owner_info = CmdEnv().entity_info(receiver)
        assert owner_info["entity_name"] == "有人机", "仅有人机可执行干扰指令。"
        ...
        return dict(CmdJamControl=action)
