...
class DemoDecision():
                ...
                if my_plane.LeftWeapon > 0:
                    for enemy_plane in threat_plane_list:
                        ...
                        if tmp_dis < dis:
                            ...
                            dis = tmp_dis
                        if my_plane.Type == 1:
                                cmd_list.append(env_cmd.make_attackparam(my_plane.ID, enemy_plane.ID, 0.8))
                        else:
                            cmd_list.append(env_cmd.make_attackparam(my_plane.ID, enemy_plane.ID, 1))
                            self.hit_enemy_list.append([enemy_plane, None])
                            enemy_plane.num_locked_missile += 1
    ...
    def get_threat_target_list(self):
        ...
        for hit_enemy in self.hit_enemy_list:
            # hit_enemy_list:被打击的敌人列表
            leader_hit = False
            # 敌有人机可以打两发 num_locked_missile:被几发导弹瞄准
