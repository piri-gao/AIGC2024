...
from agent.QYKZ_semi.tink_agent_base import Plane, Missile
...
class DemoAgent(Agent):
                ...
                if obs_agent_info['ID'] == agent_info.ID:
                    if obs_agent_info['Type'] == 3:
                        ...
                        if obs_agent_info['Identification'] != self.my_plane[0].Identification:
                            ...
                            for plane in self.my_plane:
                                if plane.Availability and plane.can_see(agent_info, see_factor=0.9,
                                                                        enemy_in_my_center=1):
                ...
                if agent_info.Type == 3 and agent_info.Identification != self.my_plane[
                    ...
                    if attacked_plane.Availability and agent_info.loss_target == False:
                        ...
                        agent_info.imaginative_update_agent_info(target_dis, target_heading, target_pitch,
                                                                 self.sim_time)
        # 更新敌机是否在敌有人机的保护下
        if len(agent_info_list) > 1 and agent_info_list[0].Identification != self.my_plane[0].Identification and \
                agent_info_list[0].Type != 3:
    ...
    def update_decision(self, obs_side, cmd_list):
        ...
        if self.sim_time <= 2:
                        ...
                        if leader_plane.wing_plane is None:
                            ...
                            for plane_id in no_missile_plane:
                                ...
                                if TSVector3.distance(plane.pos3d,
                                                      leader_plane.pos3d) < dis and plane.Type == 2 and plane.wing_who == None:
            ...
            for leader in self.my_leader_plane:
                if leader.Availability and self.sim_time - leader.last_jam > 60 and len(
                        leader.locked_missile_list) == 0:
                ...
                for jam_plane in jam_for_attack:
                    ...
                    for missile in my_missile:
                        ...
                        can_see = TSVector3.distance(jam_plane.pos3d,
                                                     target_plane.pos3d) < 1200000 - jam_turn_time * target_plane.Speed
                        if target_plane.ID not in can_jammed_enemy and missile.lost_flag == 0 and target_plane.lost_flag == 0 and can_jam and missile.loss_target == False and can_see:
                            can_jammed_enemy[target_plane.ID] = TSVector3.calheading(
                    ...
                    if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                        ...
                        if min_dis > 35000:
                            ...
                            if self.is_in_center(leader, 130000) == False:
                                turn_ratio = 2 / 3
                            if abs(leader.pi_bound(
                                    leader.Heading - total_heading - math.pi * turn_ratio)) > math.pi * turn_ratio:
                    ...
                    if leader.wing_plane:
                        ...
                        if len(leader.locked_missile_list) > 0:
                            ...
                            for missile_id in leader.locked_missile_list:
                                missile = self.get_body_info_by_id(self.missile_list, missile_id)
                                if TSVector3.distance(leader.pos3d,
                                                      missile.pos3d) < dis and self.sim_time - missile.arrive_time < 2:
                                    ...
                                    for other_enemy_plane in self.enemy_leader_plane:  # 遇到更近的敌人转移
                                        if other_enemy_plane.ID != enemy_plane.ID:
                                            if TSVector3.distance(my_plane.pos3d,
                                                                  enemy_plane.pos3d) > TSVector3.distance(
                                                    my_plane.pos3d,
                                                    other_enemy_plane.pos3d) + 800 and followed_plane_id not in other_enemy_plane.followed_plane:
                                                enemy_plane.followed_plane.remove(followed_plane_id)
                            ...
                            if followed_flag == 0:  # 回到敌机上一次出现的位置搜查
                                ...
                                if follow_plane is not None and self.is_in_center(follow_plane, center_radius=35000):
                                    ...
                                    if follow_plane.move_order == None:
                                        ...
                                        cmd_list.append(
                                            env_cmd.make_linepatrolparam(follow_plane.ID, [enemy_plane.pos3d, ],
                    ...
                    if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                        ...
                        if min_dis > 25000:
                            ...
                            if self.is_in_center(plane, 50000) == False:
                                turn_ratio = 2 / 3
                            if abs(plane.pi_bound(
                                    plane.Heading - total_heading - math.pi * turn_ratio)) > math.pi * turn_ratio:
                                total_heading = total_heading - math.pi * turn_ratio
                            else:
                                total_heading = total_heading + math.pi * turn_ratio
                        elif min_dis < 15000:
            ...
            for plane in self.my_uav_plane:
                ...
                for missile_id in plane.locked_missile_list:
                    missile = self.get_body_info_by_id(self.missile_list, missile_id)
                    if TSVector3.distance(plane.pos3d,
                                          missile.pos3d) < dis and self.sim_time - missile.arrive_time < 1 and missile.loss_target == False:
    ...
    def death_to_death(self, plane, new_plane_pos, missile):
        ...
        if TSVector3.distance(new_missile_pos, new_plane_pos) <= 240 or TSVector3.distance(missile.pos3d,
                                                                                           plane.pos3d) <= 200:
            return True
        else:
            return False
            ...
            if enemy.lost_flag == 0 and plane.can_attack(enemy, attack_dis=19000):  # 优先打击有导弹的飞机
                ...
                for missile_id in enemy.locked_missile_list:
                    ...
                    if threat_ID in attack_enemy:
                        ...
                        attack_enemy[threat_ID] = missile.fire_time
                if (threat_ID in attack_enemy.keys() and self.sim_time - attack_enemy[
                    threat_ID] > 2) or threat_ID not in attack_enemy.keys():
                    threat_plane_list.append(enemy.ID)
            ...
            for i, plane in enumerate(self.my_uav_plane):
                if i < 6:
                    if i != 5:
                        ...
                        cmd_list.append(
                            env_cmd.make_entityinitinfo(plane.ID, -135000 * self.side, 0, 9000, 300,
                                                        self.init_direction))
                else:
                    cmd_list.append(
        ...
        for my_plane in self.my_plane:
            if my_plane.Availability:
                ...
                if in_range and left_weapon:
                    ...
                    # if enemy_can_see_me == False:# or enemy_plane.wing_plane==0:
                    if enemy_can_see_me == False or enemy_plane.wing_plane == 0:
    ...
    def activate_jam(self, cmd_list):
        ...
        for i, plane in enumerate(self.my_leader_plane):
            ...
            for missile_id in plane.locked_missile_list:
                missile = self.get_body_info_by_id(self.missile_list, missile_id)
                if self.my_leader_plane[1 - i].Availability and self.sim_time - self.my_leader_plane[
                    1 - i].last_jam > 60:
                    if self.sim_time - self.my_leader_plane[i].last_jam < 60:
                        leader_turn_time = self.get_turn_time(self.my_leader_plane[1 - i], missile)
                    else:
                        leader_turn_time = min(self.get_turn_time(self.my_leader_plane[i], missile),
                                               self.get_turn_time(self.my_leader_plane[1 - i], missile))
                ...
                if (leader_turn_time + 6) * 1000 + 20000 < TSVector3.distance(plane.pos3d, missile.pos3d) < (
                        leader_turn_time + 8) * 1000 + 20000 and plane.ID not in need_jam_list:
                    need_jam_list.append(plane.ID)
                if TSVector3.distance(plane.pos3d,
                                      missile.pos3d) < dis and self.sim_time - missile.arrive_time < 3 and missile.loss_target == False:
                    dis = TSVector3.distance(plane.pos3d, missile.pos3d)
                    enemy = missile
    ...
    def save_myself(self, leader, need_jam_list, cmd_list):
        if leader.ID in need_jam_list and leader.do_jam == False:
            ...
            if self.sim_time - leader.last_jam > 60 and self.sim_time - leader.jammed >= 6 and enemy is not None:
                ...
                if TSVector3.distance(leader.pos3d, leader.close_missile.pos3d) / 1000 > acc_time and leader.can_see(
    ...
    def win_now(self):
        ...
        for plane in self.my_plane:
            ...
            if plane.Availability == 1:
                my_score += plane.AllWeapon - len(plane.used_missile_list)
                if plane.Type == 2:
                    my_score += 5
                else:
                    my_score += 60
        for plane in self.enemy_plane:
            ...
            if plane.Availability == 1:
                enemy_score += plane.AllWeapon - len(plane.used_missile_list)
                if plane.Type == 2:
                    enemy_score += 5
                else:
                    enemy_score += 60
        if len(self.enemy_plane) != 10:
            enemy_score += (2 - len(self.enemy_leader_plane)) * 64
            enemy_score += (8 - len(self.enemy_uav_plane)) * 7
