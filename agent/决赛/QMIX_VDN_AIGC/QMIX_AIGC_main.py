...
from agent.QMIX_VDN_AIGC.normalization import Normalization
...
class Runner_QMIX_AIGC:
    def __init__(self, args, env_name, number, seed):
        ...
        self.env = AIGC2Env(address=address)
    ...
    def run(self, ):
        ...
        while self.total_steps < self.args.max_train_steps:
            ...
            _, _, episode_steps = self.run_episode_aigc(evaluate=False)  # Run an episode
            self.total_steps += episode_steps

            if self.replay_buffer.current_size >= self.args.batch_size:
                a_l = self.agent_n.train(self.replay_buffer, self.total_steps)  # Training
                self.writer.add_scalar('agent_loss_{}'.format(self.env_name), a_l, global_step=self.total_step)
    ...
    def evaluate_policy(self, ):
        ...
        for _ in range(self.args.evaluate_times):
            win_tag, episode_reward, _ = self.run_episode_aigc(evaluate=True)
            if win_tag:
                win_times += 1
        ...
        # Save the win rates
        # np.save('agent/MAPPO_AIGC/data_train/{}_env_{}_number_{}_seed_{}.npy'.format(self.args.algorithm, self.env_name, self.number, self.seed), np.array(self.win_rates))
