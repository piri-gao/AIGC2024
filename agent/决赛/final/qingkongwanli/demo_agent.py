...
from agent.qingkongwanli.agent_base import Plane, Missile
from agent.qingkongwanli.tink_agent import DemoAgent as blue_agent
...
class DemoAgent(Agent):
    ...
    def __init__(self, name, config):
        ...
        self.my_blue_agent = blue_agent(name, config)
        self._init()
    ...
    def step(self,sim_time, obs_side, **kwargs):
        ...
        if self.sim_time==0:
            self.reset()
        cmd_list = self.update_decision(obs_side, cmd_list)
        return cmd_list
    ...
    def update_dead_info(self):
        for plane in self.my_plane:
            ...
            #     print(plane.Name,plane.move_order)
        ...
        for plane in self.enemy_plane:
            ...
            plane.be_seen_leader = False
            plane.be_seen = False
            for my_p in self.my_plane:
                if my_p.Availability and my_p.can_see(plane, see_factor=0.9):
                    plane.be_seen = True
            if plane.lost_flag:
            ...
            for missile in self.missile_list:
                ...
                if missile.EngageTargetID == plane.ID and missile.loss_target==False and missile.lost_flag==0:
                    short_dis, missile.arrive_time = self.shortest_distance_between_linesegment(plane, missile)
                    if missile.EngageTargetID == plane.ID and missile.lost_flag==1:
                        missile.arrive_time = self.sim_time-1
            # 更新导弹追踪信息
            for missile_id in plane.locked_missile_list.copy():
                tmp_missile = self.get_body_info_by_id(self.missile_list, missile_id)
        ...
        # 计算对方是否在我方有人机范围内
        for plane in self.my_plane:
            if plane.Availability and plane.Type==1 and self.sim_time-plane.last_jam>60:
                for other_plane in self.enemy_plane:
                    if other_plane.lost_flag==0 and plane.can_see(other_plane, see_factor=0.99, jam_dis=119000):
                        other_plane.be_seen_leader = True
        # 重新规划发射距离
        if change_dis:
        ...
        if self.revenge == False:  
            ...
            if have_enemy and (self.enemy_left_weapon == 1 or (self.sim_time > 20 * 60 -300\
                     and self.my_score<=self.enemy_score) or (self.my_left_weapon == 0 and self.my_score<=self.enemy_score)):
                self.revenge = True
    ...
    def update_decision(self, obs_side, cmd_list):
        ...
        if self.side == -1:
            cmd_list = self.my_blue_agent.step(self.sim_time, obs_side)
            return cmd_list
        if self.sim_time <= 2:
            ...
            if len(jam_for_attack)>0: 
                ...
                for jam_plane in jam_for_attack:
                    ...
                    if len(can_jammed_enemy)==0:
                        ...
                        can_jammed_enemy = {}
                        for plane in self.my_plane:
                            if (plane.ID == jam_plane.ID or jam_plane.can_see(plane,see_factor=0.99,jam_dis=119000)) and plane.follow_plane:
                        ...
                        if myjam:
                            ...
                            if jam_plane.can_see(middle_enemy,see_factor=0.6,jam_dis=117600):
                                ...
                                if jam_plane.move_order==None:
                                    ...
                                    print(jam_plane.ID, jam_enemy.move_order,"持续干扰")
                            else:
            ...
            for plane in self.my_plane:
                if plane.ID in self.free_plane and plane.Type==2:
                    ...
                    if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                        ...
                        if plane.ready_missile and min_dis>9000 and self.my_score<self.enemy_score+4:
                            continue
            ...
            for leader in self.my_leader_plane:
                ...
                if leader.ID in self.free_plane:
                    if total_dir != {"X": 0, "Y": 0, "Z": 0} and leader.do_jam==False:
                        ...
                        if self.revenge and leader.ready_missile and min_dis>10000:
                            continue
                ...
                if my_plane.ID in self.free_plane and my_plane.ready_missile:
                    ...
                    if my_plane.follow_plane == None:
                        ...
                        if new_follow_plane:
                            ...
                            dou = False
                            for enemy in self.enemy_plane:
                                if enemy.Availability and enemy.lost_flag>0 and (len(enemy.lost_missile_list)==0 or len(enemy.locked_missile_list)==0):
                                    if TSVector3.distance(my_plane.pos3d, enemy.pos3d)<10000:
                                        dou = True
                            # if new_follow_plane.lost_flag>0 and len(new_follow_plane.lost_missile_list)==0:
                            #     dou = True
                            if my_plane.be_seen==False or dou:
                                # if dou:
                                total_heading = my_plane.Heading
                                if self.sim_time%2:
                                    total_pitch = -math.pi/6
            ...
            for i, plane in enumerate(self.my_uav_plane):
                if i < 6:
                    if i != 5:
                        ...
                        cmd_list.append(
                            env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 100000 - (i+1) * 30000, 9000, 300, self.init_direction))
                    else:
                        self.spy_plane.append(plane.ID)
                        plane.is_spy = True
                        cmd_list.append(
