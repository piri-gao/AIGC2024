...
class Plane(object):
    ...
    def evade(self, enemy, total_threat_dir, cmd_list):  # 无人机躲避导弹策略
        ...
        if (enemy_pitch > 0 and enemy_pitch < relative_pitch * 0.5) or (
                enemy_pitch < 0 and enemy_pitch < relative_pitch * 0.5):
    ...
    def evade_leader(self, enemy, total_threat_dir, cmd_list):  # 有人机躲避导弹策略
        ...
        if dis / enemy.Speed < 1:
            ...
            dist_enemy = enemy.Speed
        new_enemy_pos = TSVector3.plus(enemy.pos3d,
                                       TSVector3.multscalar(TSVector3.calorientation(enemy.Heading, enemy.Pitch),
                                                            dist_enemy))
        if TSVector3.distance(new_plane_pos, new_enemy_pos) > 240:
            new_evade_pos['Z'] = self.Z
        ...
        if (abs(new_evade_pos["X"]) >= 142000 or abs(new_evade_pos["Y"]) >= 142000):
            ...
            cmd_list.append(env_cmd.make_areapatrolparam(self.ID, x, y, vertical_evade_route_list[0]["Z"], move_speed,
                                             100, self.para["move_max_speed"],
                                             self.para["move_max_acc"], self.para["move_max_g"]))
            # cmd_list.append(
            #     env_cmd.make_areapatrolparam(self.ID, self.X, self.Y, vertical_evade_route_list[0]["Z"], move_speed,
            #                                  100, self.para["move_max_speed"],
            #                                  self.para["move_max_acc"], self.para["move_max_g"]))
    ...
    def can_see(self, enemy, see_factor=1.1, jam_dis=0):  # 判断飞机是否能看到敌方实体
        ...
        relative_pitch = TSVector3.calpitch(vector)
        if dis < jam_dis and \
                abs(self.pi_bound(enemy_theta - self.Heading)) * 180 / math.pi < self.para[
            'radar_heading'] * see_factor and \
...
class Missile(object):
    ...
    def update_agent_info(self, missile_info):
        ...
        if self.Speed ** 2 + 2 * self.missile_acc * missile_info['distance'] > 0 and self.missile_acc != 0:
            self.arrive_time = missile_info['CurTime'] + arrive_flag * (math.sqrt(
                self.Speed ** 2 + 2 * self.missile_acc * missile_info['distance']) - self.Speed) / self.missile_acc
        else:
    ...
    def imaginative_update_agent_info(self, target_dis, target_heading, target_pitch, sim_time):
        ...
        if self.Speed ** 2 + 2 * self.missile_acc * target_dis > 0 and self.missile_acc != 0:
            self.arrive_time = sim_time + (
                        math.sqrt(self.Speed ** 2 + 2 * self.missile_acc * target_dis) - self.Speed) / self.missile_acc
        else:
