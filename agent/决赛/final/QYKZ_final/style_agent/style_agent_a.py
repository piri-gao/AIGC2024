...
from agent.QYKZ_final.agent_base.agent_base import Plane, Missile
...
class QYKZ_Agent(Agent):
    ...
    def update_plane_info(self):
        ...
        return no_missile_plane, threat_plane_list

    def add_view(self, cmd_list):
        need_seen_plane = []
    ...
    def attack_with_jam(self, threat_plane_list, cmd_list):
    ...
    def activate_jam(self, cmd_list):
        ...
        for i, plane in enumerate(self.my_leader_plane):
            if plane.Availability==0 or len(plane.locked_missile_list)==0:
                continue
        ...
        if len(evade_list):
            for leader_id in evade_list:
                ...
                if leader.do_turn or (leader.do_jam and TSVector3.distance(leader.pos3d, leader.close_missile.pos3d)>10000):
                    continue
                total_dir,min_dis,threat_pitch = self.synthetic_threat_vector(leader, see_dis=25000,seen=False)
    ...
    def attack_v2(self, cmd_list):
        ...
        if len(jam_for_attack)>0: 
            ...
            for jam_plane in jam_for_attack:
                ...
                jammed_missile_list = []
                for enemy in self.enemy_plane:
                    if enemy.lost_flag==0 and jam_plane.can_see(enemy,see_factor=0.99,jam_dis=110000):
                        enemy.be_seen_leader = True
                        enemy.be_seen = True
                for plane in self.my_plane:
                    if plane.ID != jam_plane.ID and jam_plane.can_see(plane, see_factor=0.99,jam_dis=119000):
                ...
                for missile_id in my_missile.copy():
                    ...
                    if target_plane.ID not in can_jammed_enemy and missile.lost_flag==0 and target_plane.Availability and can_jam and missile.loss_target==False and can_see:# and self.sim_time-target_plane.jammed>6:
                ...
                if len(can_jammed_enemy):
                    if myjam:
                        ...
                        if jam_plane.can_see(middle_enemy,see_factor=0.6,jam_dis=117600):
                            ...
                            for plane in self.my_plane:
                                if jam_plane.can_see(plane,see_factor=0.99,jam_dis=119000) and plane.ID != jam_plane.ID:
                                    plane.jammed = self.sim_time+1
                            # print("ganrao1")
                        ...
                        if jam_plane.can_see(middle_enemy,see_factor=0.6,jam_dis=117600):
                            ...
                            #     if jam_plane.can_see(plane,see_factor=0.99,jam_dis=119000) and plane.Availability:
                            #         plane.jammed = self.sim_time+1
                            # print("ganrao2")
                            if jam_plane.ID in self.free_plane:
                                self.free_plane.remove(jam_plane.ID)
    ...
    def protect_UAV(self, no_missile_plane, cmd_list):
        for plane in self.my_plane:
            if plane.ID in self.free_plane and plane.Type==2:
                if plane.ready_missile:
                    if plane.be_seen_leader:
                        total_dir, min_dis, threat_pitch = self.synthetic_threat_vector(plane, see_dis=20000,seen=False)
                    else:
                        total_dir, min_dis, threat_pitch = self.synthetic_threat_vector(plane, see_dis=7000,seen=False)
                else:
                ...
                if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                    ...
                    elif min_dis<7000 or plane.be_seen==False:
                        total_heading = math.pi/4+plane.Heading
                        # if self.sim_time%2:
    ...
    def attack_v3(self, no_missile_plane, cmd_list):
        for plane in self.my_plane:
    ...
    def protect_leader(self, cmd_list):
        for leader in self.my_leader_plane:
            if leader.ready_missile and self.revenge:
            ...
            if leader.ID in self.free_plane:
                if total_dir != {"X": 0, "Y": 0, "Z": 0} and leader.do_jam==False:
                    ...
                    if self.revenge and leader.ready_missile:
                        continue
                    ...
                    if min_dis>26000 and min_dis != 99999999:
                        ...
                        total_heading = math.pi/4+leader.Heading
                        # if self.sim_time%2:
                        #     total_pitch = -math.pi/6
                        # else:
                        #     total_pitch = math.pi/6
    ...
    def find_wing(self, cmd_list):
        for plane in self.my_plane:
            if (plane.be_in_danger or plane.follow_plane) and plane.Availability and plane.wing_plane==None and plane.be_seen==False:
                find_wing_plane = None
                for other_plane in self.my_plane:
                    if other_plane.ID != plane.ID and other_plane.be_in_danger==False and other_plane.ID in self.free_plane and other_plane.move_order==None and other_plane.wing_who==None and other_plane.follow_plane==None:
                ...
                if find_wing_plane:
                    ...
                    cmd_list.append(env_cmd.make_followparam(find_wing_plane.ID, plane.ID, find_wing_plane.para['move_max_speed'], find_wing_plane.para['move_max_acc'], find_wing_plane.para['move_max_g']))

    def add_view_v2(self, cmd_list):
        for my_plane in self.my_plane:
                ...
                if my_plane.follow_plane == None:
                    ...
                    if my_plane.follow_plane is not None and enemy_plane.lost_flag<self.get_turn_time(my_plane,enemy_plane)+2 and enemy_plane.Availability:
                        if my_plane.move_order==None:
                            ...
                            if min_dis<10000 or my_plane.be_seen==False:
                                total_heading = my_plane.Heading
                                if self.sim_time%2:
                                    total_pitch = -math.pi/6
                                else:
                                    total_pitch = math.pi/6      
                    ...
                    for other_enemy_plane in self.enemy_plane: # 遇到更近的敌人转移
                        if new_follow_plane and other_enemy_plane.ID != new_follow_plane.ID:
                            if TSVector3.distance(my_plane.pos3d, new_follow_plane.pos3d) > TSVector3.distance(my_plane.pos3d, other_enemy_plane.pos3d) + 800 \
                                ...
                                if len(other_enemy_plane.followed_plane):
                                    have_missile = False
            ...
            for missile_id in plane.locked_missile_list.copy():
                ...
                for m_p in self.my_plane:
                    if m_p.can_see(tmp_missile, see_factor=0.97) and m_p.lost_flag==0:
                        be_seen = True
                if (tmp_missile.lost_flag and (self.sim_time - tmp_missile.arrive_time > 3 or (be_seen and self.sim_time - tmp_missile.arrive_time>1))) or tmp_missile.loss_target:
    ...
    def init_pos(self, cmd_list):
        ...
        if self.sim_time == 2:
            ...
            # 初始化有人机位置
            cmd_list.append(
                env_cmd.make_entityinitinfo(leader_plane_1.ID, -125000 * self.side, 30000, 9000, 400, self.init_direction))
            cmd_list.append(
                env_cmd.make_entityinitinfo(leader_plane_2.ID, -125000 * self.side, -30000, 9000, 400, self.init_direction))
            for i, plane in enumerate(self.my_uav_plane):
                if i < 6:
                    if i != 5:
                        ...
                        if i == 0:
                            cmd_list.append(
                                env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 50000, 9000, 300, self.init_direction))
                        if i == 1:
                            cmd_list.append(
                                env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 40000, 9000, 300, self.init_direction))
                        if i == 2:
                            cmd_list.append(
                                env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 10000, 9000, 300, self.init_direction))
                        if i == 3:
                            cmd_list.append(
                                env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, -40000, 9000, 300, self.init_direction))
                        if i == 4:
                    ...
                    cmd_list.append(
                        env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 60000 - (i+1)%3 * 40000, 9000, 300, self.init_direction))
    ...
    def synthetic_threat_vector(self, leader, see_dis=25000, seen=True):
        ...
        for enemy in self.enemy_plane:
            if enemy.lost_flag==0 and enemy.ready_missile>0:
    ...
    def init_moving_v2(self, cmd_list):
    ...
    def init_move(self, cmd_list):
        ...
        for plane_ID in self.free_plane:
            plane = self.get_body_info_by_id(self.my_plane, plane_ID)
            if plane.Type==2 and 0<len(self.enemy_plane)<10:
                if self.sim_time%2:
    ...
    def update_decision(self, obs_side, cmd_list):
        ...
        if self.sim_time <= 2:
            ...
            self.free_plane = []
            # 更新飞机信息
            no_missile_plane, threat_plane_list = self.update_plane_info()
            # 增加视野
            self.add_view(cmd_list)
            # 开火
            self.attack_with_jam(threat_plane_list, cmd_list)
            # 干扰
            self.activate_jam(cmd_list)
            # 攻击
            self.attack_v2(cmd_list)
            # 保护无人机
            self.protect_UAV(no_missile_plane, cmd_list)
            # 攻击
            self.attack_v3(no_missile_plane, cmd_list)
            # 保护有人机
            self.protect_leader(cmd_list)
            # 集火
            self.find_wing(cmd_list)
            # 扩大视野
            self.add_view_v2(cmd_list)
            self.add_view_v3(cmd_list)
            # 保护无人机
            self.evade_UAV(cmd_list) 
            # 初始化移动
            self.init_moving_v2(cmd_list)
