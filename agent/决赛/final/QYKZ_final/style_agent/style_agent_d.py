...
from agent.QYKZ_final.agent_base.agent_base_v3 import Plane, Missile

class Agent_v3(Agent):

    def __init__(self, name, config):
        super(Agent_v3, self).__init__(name, config['side'])
        self._init()
    ...
    def update_dead_info(self):
        for plane in self.my_plane:
            ...
            for missile_id in plane.locked_missile_list:
                ...
                if TSVector3.distance(plane.pos3d,
                                      missile.pos3d) < dis and self.sim_time - missile.arrive_time < 2 and missile.loss_target == False:
            ...
            if plane.lost_flag:
                if len(plane.locked_missile_list) > 0:
                    ...
                    for missile_id in plane.lost_missile_list:
                        ...
                        dis = TSVector3.distance(tmp_missile.pos3d, plane.pos3d)
                        next_plane_pos = TSVector3.plus(plane.pos3d, TSVector3.multscalar(new_dir,
                                                                                          dis / tmp_missile.Speed * plane.Speed))
        ...
        for plane in self.enemy_plane:
            ...
            if plane.Availability:
                for wing_plane in self.enemy_plane:
                    if wing_plane.Availability and plane.ID != wing_plane.ID and wing_plane.can_see(plane,
                                                                                                    see_factor=1):
                        plane.wing_plane += 1
        spy_enemy = None
        dis = 999999999
    ...
    def get_all_body_list(self, agent_info_list, obs_list, cls):
                ...
                if obs_agent_info['ID'] == agent_info.ID:
                    if obs_agent_info['Type'] == 3:
                        if obs_agent_info['Identification'] != self.my_plane[0].Identification:
                            ...
                            attacked_plane = self.get_body_info_by_id(self.enemy_plane,
                                                                      obs_agent_info['EngageTargetID'])
        ...
        for agent_info in agent_info_list:
            ...
            if not agent_in_list:
                ...
                if agent_info.Type == 3 and agent_info.Identification != self.my_plane[
                    0].Identification and agent_info.loss_target == False:
    ...
    def death_to_death(self, plane, new_plane_pos, missile, radius=180):
        ...
        if TSVector3.distance(new_missile_pos, new_plane_pos) <= radius or TSVector3.distance(missile.pos3d,
                                                                                              plane.pos3d) <= radius - 40:
            return True
        else:
            return False
    ...
    def all_death(self, plane, cmd_list):
        ...
        if len(threat_plane_list):
            threat_plane_list = sorted(threat_plane_list,
                                       key=lambda d: self.get_body_info_by_id(self.enemy_plane, d).ready_missile,
                                       reverse=True)
            threat_plane_list = sorted(threat_plane_list,
                                       key=lambda d: self.get_body_info_by_id(self.enemy_plane, d).wing_plane,
                                       reverse=False)
            threat_plane_list = sorted(threat_plane_list,
                                       key=lambda d: self.get_body_info_by_id(self.enemy_plane, d).Type, reverse=False)
    ...
    def init_pos(self, cmd_list):
        ...
        if self.sim_time == 2:
            ...
            # 初始化有人机位置
            cmd_list.append(
                env_cmd.make_entityinitinfo(leader_plane_1.ID, -135000 * self.side, 50000, 9000, 400,
                                            self.init_direction))
            cmd_list.append(
                env_cmd.make_entityinitinfo(leader_plane_2.ID, -135000 * self.side, -50000, 9000, 400,
                                            self.init_direction))
            for i, plane in enumerate(self.my_uav_plane):
                if i < 6:
                    if i != 5:
                        cmd_list.append(
                            env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 150000 - (i + 1) * 50000, 9000,
                                                        300, self.init_direction))
                    else:
                        self.spy_plane = plane.ID
                        plane.is_spy = True
                        cmd_list.append(
                            env_cmd.make_entityinitinfo(plane.ID, -125000 * self.side, 0, 9000, 300,
                                                        self.init_direction))
                else:
                    cmd_list.append(
                        env_cmd.make_entityinitinfo(plane.ID, -145000 * self.side, 75000 - (i + 1) % 3 * 50000, 9000,
                                                    300, self.init_direction))
    ...
    def init_move(self, free_plane, cmd_list):
        for plane_ID in free_plane:
            ...
            if plane.Type == 2 and len(self.enemy_plane):
                ...
                relative_pitch = 0
            if (self.side == 1 and plane.X > self.bound * self.side) or (
                    self.side == -1 and plane.X < self.bound * self.side):
            ...
            if init_direction != self.init_direction or len(self.enemy_leader_plane) == 2:
                route_list = [
                    {"X": (random.random() * 2 - 1) * 3000, "Y": (random.random() * 2 - 1) * 3000, "Z": new_pos['Z']}, ]
            if len(self.enemy_leader_plane) and self.is_in_center(plane, center_radius=40000) == False:
                route_list = [
                    {"X": (random.random() * 2 - 1) * 3000, "Y": (random.random() * 2 - 1) * 3000, "Z": 9000}, ]
            # 没有敌人，朝对方中心点移动
                ...
                if plane.is_spy:  # 暗度陈仓
                    if len(self.enemy_leader_plane):
                        ...
                        if spy_enemy != None:
                            ...
                            new_pos = TSVector3.plus(spy_enemy.pos3d,
                                                     TSVector3.multscalar(new_dir, spy_enemy.Speed * 6))
                            new_pos['Z'] = 2000
                            route_list = [new_pos, ]
                    else:
    ...
    def can_attack_plane(self, enemy_plane):
        ...
        if len(enemy_plane.locked_missile_list) or (self.sim_time > 15 * 60 and self.my_score < self.enemy_score):
            attack_only_my = True
        for my_plane in self.my_plane:  # 有进入必杀的敌机，直接必杀
            if my_plane.Availability and my_plane.ready_missile > 0 and (
                    enemy_plane.ID not in my_plane.ready_attack or attack_only_my):
                ...
                if in_range and left_weapon:
                    ...
                    for enemy in self.enemy_plane:
                        if enemy.can_see(attack_plane, see_factor=1):
                            enemy_can_see_me = True
                    if (enemy_can_see_me == False and enemy_plane.wing_plane == 0) or (
                            self.my_score > self.enemy_score):
        ...
        can_attack_list = [key for key, value in sorted(can_attack_dict.items(),
                                                        key=lambda d: self.get_body_info_by_id(self.my_plane,
                                                                                               d[0]).Type,
                                                        reverse=False)]
    ...
    def activate_jam(self, cmd_list):
        ...
        for leader in self.my_leader_plane:
            ...
            for enemy in self.enemy_plane:
                if enemy.lost_flag == 0 and enemy.can_see(leader):
                    dis = leader.pi_bound(
                        TSVector3.calheading(TSVector3.minus(enemy.pos3d, leader.pos3d)) - leader.Heading)
        ...
        if len(need_jam_list):
            ...
            if leader1.close_missile and (
                    leader1.close_missile.init_dis < 6000 or self.death_to_death(leader1, new_plane_pos,
                                                                                 leader1.close_missile)):
            ...
            if leader2.close_missile and (
                    leader2.close_missile.init_dis < 6000 or self.death_to_death(leader2, new_plane_pos,
                                                                                 leader2.close_missile)):
    ...
    def save_myself(self, leader, need_jam_list, cmd_list):
        if leader.ID in need_jam_list and leader.do_jam == False:
            ...
            if self.sim_time - leader.last_jam > 60 and self.sim_time - leader.jammed >= 6 and enemy is not None and check_turn:
                ...
                if enemy.lost_flag == 0 and TSVector3.distance(leader.pos3d,
                                                               leader.close_missile.pos3d) / 1000 > acc_time and leader.can_see(
                        enemy, see_factor=0.6, jam_dis=117600):
                    leader.do_jam = True
                    ...
                    can_jam = True
                elif TSVector3.distance(leader.pos3d,
                                        leader.close_missile.pos3d) / 1000 > acc_time + self.get_turn_time(leader,
                                                                                                           enemy,
                                                                                                           see_factor=0.6):
            ...
            if can_jam == False:
                ...
                if leader.close_missile and (
                        leader.close_missile.init_dis < 6000 or self.death_to_death(leader, new_plane_pos,
                                                                                    leader.close_missile)):
    ...
    def plane_can_jam(self, need_jam_plane, jam_plane, evade_list, cmd_list):
        if need_jam_plane.middle_enemy_plane is not None:
            ...
            if enemy.lost_flag == 0:
                can_jam = False
                # 别人可以帮忙吗
                if not need_jam_plane.can_see(enemy, see_factor=0.6, jam_dis=117600) and jam_plane.can_see(enemy,
                                                                                                           see_factor=0.9,
                                                                                                           jam_dis=117600):
                    if jam_plane.ID not in evade_list:
                        ...
                        cmd_list.append(env_cmd.make_followparam(jam_plane.ID, enemy.ID, jam_plane.Speed,
                                                                 jam_plane.para['move_max_acc'],
                                                                 jam_plane.para['move_max_g']))
                        can_jam = True
                        ...
                        can_jam = False
                # 靠自己看行不行
                elif need_jam_plane.can_see(enemy, see_factor=0.6,
                                            jam_dis=117600) and self.sim_time - need_jam_plane.last_jam > 60 \
                        and self.sim_time - need_jam_plane.jammed >= 6 and TSVector3.distance(
                    need_jam_plane.close_missile.pos3d, need_jam_plane.pos3d) > 15000 \
                        and self.check_jam_after_turn(need_jam_plane, need_jam_plane,
                                                      need_jam_plane.middle_enemy_plane):  # 有人机需要进行转弯判断
    ...
    def find_way_jam(self, jam_plane, middle_enemy_id, cmd_list):
        ...
        if jam_enemy.lost_flag == 0:
            ...
            cmd_list.append(
                env_cmd.make_followparam(jam_plane.ID, jam_enemy.ID, jam_plane.Speed, jam_plane.para['move_max_acc'],
                                         jam_plane.para['move_max_g']))
        else:
            if jam_plane.move_order == None:
            ...
            cmd_list.append(
                env_cmd.make_linepatrolparam(jam_plane.ID, [jam_enemy.pos3d, ], jam_plane.para["move_max_speed"],
    ...
    def is_in_legal_area(self, plane):
        ...
        if abs(next_pos['X']) > 150000 or abs(next_pos['Y']) > 150000 or next_pos['Z'] < 2000 or next_pos['Z'] > \
                plane.para['area_max_alt']:
            return False
    ...
    def update_plane_list(self):
        ...
        if len(threat_plane_list):
            ...
            threat_plane_list = sorted(threat_plane_list,
                                    ...
                                    reverse=False)
            threat_plane_list = sorted(threat_plane_list,
                                    key=lambda d: self.get_body_info_by_id(self.enemy_plane, d).Type,
                                    reverse=False)
        return free_plane, no_missile_plane, threat_plane_list
    
    def assign_wing_plane(self, free_plane, no_missile_plane):
                ...
                if leader_plane.Availability:
                    ...
                    if leader_plane.wing_plane is None:
                        ...
                        for plane_id in no_missile_plane:
                            plane = self.get_body_info_by_id(self.my_plane, plane_id)
                            if TSVector3.distance(plane.pos3d,
                                                leader_plane.pos3d) < dis and plane.Type == 2 and plane.wing_who == None and plane.is_spy == False:
                        ...
                        if leader_plane.wing_plane is not None:
                            self.get_body_info_by_id(self.my_plane,
                                                    leader_plane.wing_plane).wing_who = leader_plane.ID
                            ...
                            for plane_id in free_plane:
                                plane = self.get_body_info_by_id(self.my_plane, plane_id)
                                if plane.Type == 2 and TSVector3.distance(plane.pos3d,
                                                                        leader_plane.pos3d) < dis and plane.ready_missile <= have_missile and plane.wing_who == None:
                                    dis = TSVector3.distance(plane.pos3d, leader_plane.pos3d)
        ...
        if len(jam_for_attack) > 0:
            ...
            for jam_plane in jam_for_attack:
                ...
                for missile_id in my_missile.copy():
                    ...
                    can_jam = jam_turn_time * missile.Speed < TSVector3.distance(missile.pos3d,
                                                                                target_plane.pos3d) < (
                                        jam_turn_time + 6) * missile.Speed
                    can_see = TSVector3.distance(jam_plane.pos3d,
                                                target_plane.pos3d) < 120000 - jam_turn_time * target_plane.Speed
                    if target_plane.ID not in can_jammed_enemy and missile.lost_flag == 0 and target_plane.lost_flag == 0 and can_jam and missile.loss_target == False and can_see:
                        can_jammed_enemy[target_plane.ID] = jam_plane.pi_bound(TSVector3.calheading(
                            TSVector3.minus(target_plane.pos3d, jam_plane.pos3d)) - jam_plane.Heading)
    ...
    def attack_further(self, cmd_list, threat_plane_list):
        ...
        while (can_attack_flag == True):
            ...
            for threat_plane_id in threat_plane_list:
                ...
                if self.my_score > self.enemy_score:
                    ...
                    missile_num = 0
                if threat_plane.Type == 2 and (
                        len(threat_plane.locked_missile_list) > missile_num or self.my_score <= self.enemy_score):
                    continue
                ...
                if attack_plane is not None:
                    ...
                    if self.my_score < self.enemy_score + 36 or (self.sim_time > 15 * 60 and \
                                                                (self.my_score < self.enemy_score or (
                                                                        self.my_score == self.enemy_score \
                    ...
                    if TSVector3.distance(attack_plane.pos3d,
                                        threat_plane.pos3d) / 1000 + self.sim_time + 2 > 20 * 60:
                        can_attack_now = False
                    if self.my_score - 1 == self.enemy_score and self.my_center_time < self.enemy_center_time:
                        can_attack_now = False
                    if self.my_score == self.enemy_score and (
                            self.my_center_time > self.enemy_center_time * 2 or self.my_center_time > self.enemy_center_time + (
                            20 * 60 - self.sim_time) * self.live_enemy_leader):
                        can_attack_now = False
                    if can_attack_now:
        ...
        for threat_plane_id in threat_plane_list:
            threat_plane = self.get_body_info_by_id(self.enemy_plane, threat_plane_id)
            if len(threat_plane.ready_attacked) > 1 or (
                    len(threat_plane.locked_missile_list) and len(threat_plane.ready_attacked)) or (
                    self.sim_time > 13 * 60 and (self.my_score < self.enemy_score or (
                    self.my_score == self.enemy_score and self.my_center_time < self.enemy_center_time))):
    ...
    def leader_out(self, free_plane, cmd_list):
        for leader in self.my_leader_plane:
            need_wing = False
            if leader.ID in free_plane and (self.my_left_weapon > 2 or len(leader.used_missile_list) == 4):
                ...
                if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                    ...
                    if self.my_score > self.enemy_score or self.enemy_strategy() or self.sim_time < 13 * 60:  # self.enemy_formation()==False:
                        center_radius = 120000
                    elif self.my_score == self.enemy_score and (
                            self.my_center_time < self.enemy_center_time * 2 or self.my_center_time < self.enemy_center_time + (
                            20 * 60 - self.sim_time) * self.live_enemy_leader):
                        center_radius = 45000
                    else:
                    ...
                    if self.is_in_center(leader, center_radius) == False:
                        tmp_dir2 = {"X": (random.random() * 2 - 1) * 3000, "Y": (random.random() * 2 - 1) * 3000,
                                    "Z": 9000}
                    ...
                    if leader.move_order == None:
                        ...
                        cmd_list.append(env_cmd.make_linepatrolparam(leader.ID, straight_evade_route_list,
                                                                    move_speed, leader.para["move_max_acc"],
                                                                    leader.para["move_max_g"]))
                    if leader.Availability and leader.wing_plane:
                        ...
                        if TSVector3.distance(wing_plane.pos3d, leader.pos3d) > 35000:
                            cmd_list.append(env_cmd.make_followparam(wing_plane.ID, leader.ID,
                                                                    wing_plane.para["move_max_speed"],
                                                                    wing_plane.para['move_max_acc'],
                                                                    wing_plane.para['move_max_g']))
                        else:
                    ...
                    for plane in self.my_plane:
                        if plane.ID != leader.ID and plane.Availability:
                            if plane.follow_plane != None and plane.ready_missile and plane.ID not in support_uav_plane_list:
                                dis = leader.pi_bound(TSVector3.calheading(
                                    TSVector3.minus(plane.pos3d, leader.pos3d)) - leader.Heading)
                                support_uav_plane_list[plane.ID] = dis
                    if len(support_uav_plane_list):
                        support_uav_plane_list = [key for key, value in
                                                sorted(support_uav_plane_list.items(), key=lambda d: d[1])]
                        middile_my_plane = support_uav_plane_list[int(len(support_uav_plane_list) / 2)]
                        if leader.move_order == None:
                            leader.move_order = "提前为无人机提供隐身视野支持"
                            free_plane.remove(leader.ID)
                            cmd_list.append(
                                env_cmd.make_followparam(leader.ID, middile_my_plane, leader.para["move_max_speed"],
                                                        leader.para['move_max_acc'], leader.para['move_max_g']))
                        ...
                        for enemy in self.enemy_plane:
                            if self.enemy_left_weapon > 0:
                                if enemy.Availability and enemy.ready_missile and enemy.ID not in detect_enemy_list:
                                    dis = leader.pi_bound(TSVector3.calheading(
                                        TSVector3.minus(enemy.pos3d, leader.pos3d)) - leader.Heading)
                        ...
                        if len(detect_enemy_list):
                            detect_enemy_list = [key for key, value in
                                                sorted(detect_enemy_list.items(), key=lambda d: d[1])]
                            ...
                            if leader.move_order == None:
                                ...
                                if middile_enemy_plane.lost_flag == 0:
                                    cmd_list.append(env_cmd.make_followparam(leader.ID, middile_enemy_plane.ID,
                                                                            leader.para["move_max_speed"],
                                                                            leader.para['move_max_acc'],
                                                                            leader.para['move_max_g']))
                                else:
                                    cmd_list.append(
                                        env_cmd.make_linepatrolparam(leader.ID, [middile_enemy_plane.pos3d, ],
                                                                    leader.para["move_max_speed"],
                                                                    leader.para["move_max_acc"],
                                                                    leader.para["move_max_g"]))
                    need_wing = True
            ...
            if leader.Availability and need_wing:
                if leader.wing_plane:
                    wing_plane = self.get_body_info_by_id(self.my_plane, leader.wing_plane)
                    if wing_plane.close_missile != None and TSVector3.distance(wing_plane.pos3d,
                                                                            wing_plane.close_missile.pos3d) < 6000:
                        continue
                    if wing_plane.ID in free_plane:
                        free_plane.remove(wing_plane.ID)
                    if len(leader.locked_missile_list) > 0:
                        ...
                        if follow_missile != None and follow_missile.lost_flag == 0:
                            cmd_list.append(env_cmd.make_followparam(wing_plane.ID, follow_missile.ID,
                                                                    wing_plane.para["move_max_speed"],
                                                                    wing_plane.para['move_max_acc'],
                                                                    wing_plane.para['move_max_g']))
                            ...
                            next_pos = TSVector3.plus(follow_missile.pos3d,
                                                    TSVector3.multscalar(total_dir, distance))
                            cmd_list.append(env_cmd.make_linepatrolparam(wing_plane.ID, [next_pos, ],
                                                                        wing_plane.para["move_max_speed"],
                                                                        wing_plane.para["move_max_acc"],
                                                                        wing_plane.para["move_max_g"]))
    ...
    def attack_enemy_leader(self, free_plane, cmd_list):
        if len(free_plane) > 0:
            for enemy_plane in self.enemy_leader_plane:
                for followed_plane_id in enemy_plane.followed_plane.copy():
                    ...
                    if followed_plane_id in free_plane and my_plane.ready_missile > 0:
                        if self.is_in_center(my_plane, center_radius=100000) == False and len(
                                enemy_plane.followed_plane) > 1:
                            ...
                            for other_enemy_plane in self.enemy_leader_plane:  # 遇到更近的敌人转移
                                if other_enemy_plane.ID != enemy_plane.ID:
                                    if TSVector3.distance(my_plane.pos3d, enemy_plane.pos3d) > TSVector3.distance(
                                            my_plane.pos3d, other_enemy_plane.pos3d) + 800 \
            ...
            for enemy_plane in self.enemy_leader_plane:
                ...
                if enemy_plane.Availability:
                    ...
                    enemy_plane.followed_plane = sorted(enemy_plane.followed_plane,
                                                        key=lambda d: TSVector3.distance(
                                                            self.get_body_info_by_id(self.my_plane, d).pos3d,
                                                            enemy_plane.pos3d), reverse=True)
                    for plane_id in enemy_plane.followed_plane.copy():
                        ...
                        if plane_id in free_plane and enemy_plane.lost_flag == 0:
                            ...
                            if my_plane.move_order == None:
                                ...
                                cmd_list.append(env_cmd.make_followparam(my_plane.ID, enemy_plane.ID,
                                                                        my_plane.para["move_max_speed"],
                                                                        my_plane.para['move_max_acc'],
                                                                        my_plane.para['move_max_g']))
                    ...
                    if followed_flag == 0:  # 回到敌机上一次出现的位置搜查
                        ...
                        if follow_plane is not None and enemy_plane.lost_flag < self.get_turn_time(follow_plane,
                                                                                                enemy_plane) + 2:
                            free_plane.remove(follow_plane.ID)
                            if follow_plane.move_order == None:
                                follow_plane.move_order = "搜查敌有人机"
                                cmd_list.append(env_cmd.make_linepatrolparam(follow_plane.ID, [enemy_plane.pos3d, ],
                                                                            follow_plane.para["move_max_speed"],
                                                                            follow_plane.para["move_max_acc"],
                                                                            follow_plane.para["move_max_g"]))
                        elif follow_plane is not None:
                            follow_plane.follow_plane = None
    
    def optimize_view(self, free_plane, cmd_list):
        if len(self.enemy_leader_plane) == 2:
            # 先给我方有人机视野
            for enemy in self.enemy_plane:
                for leader in self.my_leader_plane:
                    if leader.Availability and enemy.can_see(leader) and len(
                            enemy.followed_plane) == 0 and enemy.ready_missile > 0:
                        dis = 99999999
                        follow_plane = None
                        for my_plane in self.first_formation:
                            if my_plane.ID in free_plane:
                                if TSVector3.distance(my_plane.pos3d,
                                                    enemy.pos3d) < dis and my_plane.is_spy == False:
                                    dis = TSVector3.distance(my_plane.pos3d, enemy.pos3d)
                                    follow_plane = my_plane
                        if follow_plane is not None and enemy.lost_flag < self.get_turn_time(follow_plane,
                                                                                            enemy) + 2:
                            ...
                            if 0 < enemy.lost_flag < self.get_turn_time(follow_plane, enemy) + 2:
                                cmd_list.append(env_cmd.make_linepatrolparam(follow_plane.ID, [enemy.pos3d, ],
                                                                            follow_plane.para["move_max_speed"],
                                                                            follow_plane.para["move_max_acc"],
                                                                            follow_plane.para["move_max_g"]))
                            else:
                                cmd_list.append(env_cmd.make_followparam(follow_plane.ID, enemy.ID,
                                                                        follow_plane.para["move_max_speed"],
                                                                        follow_plane.para['move_max_acc'],
                                                                        follow_plane.para['move_max_g']))
                ...
                if my_plane.ID in free_plane:
                    if my_plane.follow_plane == None and my_plane.is_spy == False:
                        ...
                        for enemy_plane in self.enemy_uav_plane:
                            if enemy_plane.Availability and len(
                                    enemy_plane.followed_plane) == 0 and enemy_plane.ready_missile > 0:
                                if TSVector3.distance(enemy_plane.pos3d, my_plane.pos3d) < dis:
                                    dis = TSVector3.distance(enemy_plane.pos3d, my_plane.pos3d)
                                    my_plane.follow_plane = enemy_plane.ID
                        if my_plane.follow_plane is not None and enemy_plane.lost_flag < self.get_turn_time(
                                my_plane, enemy_plane) + 2:
                            enemy_plane = self.get_body_info_by_id(self.enemy_plane, my_plane.follow_plane)
                            ...
                            if my_plane.move_order == None:
                                ...
                                if 0 < enemy_plane.lost_flag < self.get_turn_time(my_plane,
                                                                                enemy_plane) + 2 and enemy.Availability:
                                    cmd_list.append(env_cmd.make_linepatrolparam(my_plane.ID, [enemy_plane.pos3d, ],
                                                                                my_plane.para["move_max_speed"],
                                                                                my_plane.para["move_max_acc"],
                                                                                my_plane.para["move_max_g"]))
                                else:
    ...
    def UAV_out(self, free_plane, cmd_list):
        if len(self.enemy_leader_plane) > 0:
            # 无人机脱离近距离攻击范围
            for plane in self.my_plane:
                if plane.ID in free_plane and plane.Type == 2:
                    total_dir, min_dis = self.synthetic_threat_vector(plane, see_dis=15000, seen=True)
                    if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                        if self.is_in_center(plane, 100000) == False:
                            tmp_dir2 = {"X": (random.random() * 2 - 1) * 3000,
                                        "Y": (random.random() * 2 - 1) * 3000, "Z": 9000}
                        ...
                        if min_dis > 15000:
                            ...
                            if self.is_in_center(plane, 110000) == False:
                                ...
                                turn_ratio = 4 / 9
                            # 转弯方向需要重新判断？
                            total_threat_dir, min_dis2 = self.synthetic_threat_vector(plane, see_dis=20000,
                                                                                    seen=False)
                            total_threat_heading = TSVector3.calheading(total_threat_dir)
                        ...
                        if evade_pos['Z'] < 2000:
                            ...
                            evade_pos['Z'] = plane.Z - math.sin(abs(total_pitch)) * distance
                        straight_evade_route_list = [
                            {"X": evade_pos["X"], "Y": evade_pos["Y"], "Z": evade_pos['Z']}, ]
                        if plane.move_order == None:
                        ...
                        cmd_list.append(env_cmd.make_linepatrolparam(plane.ID, straight_evade_route_list,
                                                                    move_speed, plane.para["move_max_acc"],
                                                                    plane.para["move_max_g"]))
    
    def UAV_evade(self, cmd_list):
        for plane in self.my_uav_plane:
            if plane.close_missile is None or (
                    TSVector3.distance(plane.close_missile.pos3d, plane.pos3d) >= 6000 and plane.wing_who != None):
                continue
    ...
    def update_decision(self, obs_side, cmd_list):
        ...
        if self.sim_time <= 2:
            ...
            free_plane, no_missile_plane, threat_plane_list = self.update_plane_list()
            
            # 分配僚机
            self.assign_wing_plane(free_plane, no_missile_plane)
            
            # 干扰
            self.activate_jam(cmd_list)

            # 攻击
            self.jam_attack(cmd_list)
            

            # 干扰下开火
            self.attack_further(cmd_list, threat_plane_list)
            
            # 保护有人机
            self.leader_out(free_plane, cmd_list)
            
            # 斩首
            self.attack_enemy_leader(free_plane, cmd_list)
            
            # 优化视野
            self.optimize_view(free_plane, cmd_list)
            
            self.UAV_out(free_plane, cmd_list)
            
            # UAV保护
            self.UAV_evade(cmd_list)
