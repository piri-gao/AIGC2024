...
from agent.QYKZ_final.agent_base.agent_base_v2 import Plane, Missile

class Agent_v2(Agent):

    def __init__(self, name, config):
        super(Agent_v2, self).__init__(name, config['side'])
        self._init()
    ...
    def get_available_planes(self):
        ...
        for my_plane in self.my_plane:
            if not my_plane.locked_missile_list and my_plane.Availability:
                if my_plane.Type==1 and (my_plane.do_jam or my_plane.do_turn):
                    continue
                free_planes.append(my_plane)
                if my_plane.ready_missile == 0 and my_plane.Type == 2:
                    no_missile_planes.append(my_plane.ID)
        free_planes = sorted(free_planes, key=lambda d: d.ready_missile, reverse=False)
        free_planes = [plane.ID for plane in sorted(free_planes, key=lambda d: d.Type, reverse=True)]
        return free_planes, no_missile_planes


    def get_threat_planes(self):
        threat_plane_list = [enemy.ID for enemy in self.enemy_plane if enemy.lost_flag == 0]
        if len(threat_plane_list) > 0 :
    ...
    def find_available_wing_plane(self, free_planes, leader_plane, see_factor=0.9):
    ...
    def assign_wing_planes(self, free_plane_ids, no_missile_plane_ids):
        for leader_plane in self.my_leader_plane:
            ...
            wing_plane_id = leader_plane.wing_plane
            if wing_plane_id is not None:
                wing_plane = self.get_body_info_by_id(self.my_plane, wing_plane_id)

                if wing_plane.Availability and leader_plane.locked_missile_list and wing_plane_id in free_plane_ids:
                    free_plane_ids.remove(wing_plane_id)
                    continue
                else:
                    available_wing_plane = self.find_available_wing_plane(free_plane_ids, leader_plane)
                    if available_wing_plane:
                        wing_plane.wing_who = None
                        leader_plane.wing_plane = None
                        continue

                if wing_plane_id not in free_plane_ids or (wing_plane.ready_missile != 0 and no_missile_plane_ids):
                    wing_plane.wing_who = None
                    leader_plane.wing_plane = None

            if leader_plane.wing_plane is None:
                candidate_wing_plane_id = None
                min_distance = float('inf')

                for plane_id in no_missile_plane_ids + free_plane_ids:
                    plane = self.get_body_info_by_id(self.my_plane, plane_id)
                    distance = TSVector3.distance(plane.pos3d, leader_plane.pos3d)

                    if plane.Type == 2 and distance < min_distance and plane.wing_who is None and plane.is_spy==False:
                        min_distance = distance
                        candidate_wing_plane_id = plane_id

                if candidate_wing_plane_id is not None:
                    self.get_body_info_by_id(self.my_plane, candidate_wing_plane_id).wing_who = leader_plane.ID

                    if candidate_wing_plane_id in no_missile_plane_ids:
                        no_missile_plane_ids.remove(candidate_wing_plane_id)
                    if candidate_wing_plane_id in free_plane_ids:
                        free_plane_ids.remove(candidate_wing_plane_id)

                    leader_plane.wing_plane = candidate_wing_plane_id

    def jam_attack(self, jam_for_attack, cmd_list):
    ...
    def should_continue_attack(self, threat_plane, my_score, enemy_score):
        if my_score > enemy_score:
            missile_num = 4 if my_score > enemy_score + 36 else 1
        else:
            missile_num = 0

        return threat_plane.Type == 2 and len(threat_plane.locked_missile_list) > missile_num


    def can_attack_now(self, threat_ID, attack_enemy, my_score, enemy_score, my_center_time, enemy_center_time, sim_time, live_enemy_leader):
        if threat_ID in attack_enemy.keys() and sim_time - attack_enemy[threat_ID] < self.calculate_cold_time(my_score, enemy_score):
            return False

        if my_score - 1 == enemy_score and my_center_time < enemy_center_time:
            return False

        if my_score == enemy_score and (my_center_time > enemy_center_time * 2 or my_center_time > enemy_center_time + (20 * 60 - sim_time) * live_enemy_leader):
            return False

        return True


    def calculate_cold_time(self, my_score, enemy_score):
        if my_score < enemy_score + 36:
            return 0
        else:
            return 3
    ...
    def leader_view(self, free_plane, cmd_list):
        for enemy in self.enemy_plane:
            for leader in self.my_leader_plane:
    ...
    def other_view(self, free_plane, cmd_list):
        for my_plane in self.first_formation:
            if my_plane.ID in free_plane:
                if my_plane.follow_plane == None and my_plane.is_spy==False:
                    ...
                    if my_plane.follow_plane is not None:
                        ...
                        if enemy_plane.lost_flag<self.get_turn_time(my_plane,enemy_plane)+2:
    ...
    def update_decision(self, obs_side, cmd_list):
        ...
        if self.sim_time <= 2:
            ...
            free_plane, no_missile_plane = self.get_available_planes()
            threat_plane_list = self.get_threat_planes()
            # 分配僚机
            if len(self.enemy_plane):
                self.assign_wing_planes(free_plane, no_missile_plane)
            # 实施干扰
            self.activate_jam(cmd_list)
            # 筹划攻击
            jam_for_attack = [leader for leader in self.my_leader_plane if leader.Availability and self.sim_time - leader.last_jam > 60 and leader.do_jam==False]
            if jam_for_attack:
                self.jam_attack(jam_for_attack, cmd_list)
            # 干扰下攻击
            ...
            if len(self.enemy_leader_plane)==2:
                # 有人机view
                self.leader_view(free_plane, cmd_list)
                # 其他飞机view
                self.other_view(free_plane, cmd_list)  
            # 脱离战场            
