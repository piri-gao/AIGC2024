...
from agent.QYKZ_final.agent_base.agent_base_v6 import Plane, Missile

class Agent_v6(Agent):

    def __init__(self, name, config):
        super(Agent_v6, self).__init__(name, config['side'])
        self._init()
    ...
    def assign_wing(self, free_plane, no_missile_plane):
    ...
    def view_support(self, free_plane, cmd_list):
        need_seen_plane = []
        for plane in self.my_plane:
    ...
    def assign_wing(self, no_missile_plane, free_plane):
        if len(self.enemy_plane):
            for leader_plane in self.my_leader_plane:
                if leader_plane.Availability:
                    if leader_plane.wing_plane != None:
    ...
    def attack_with_jam_0(self, free_plane, cmd_list):
        jam_for_attack = []
        for leader in self.my_leader_plane:
            # 如果在跑路时也能提供干扰支持
    ...
    def protect_leader(self, free_plane, cmd_list):
        for leader in self.my_leader_plane:
            need_wing = False
    ...
    def UAV_away(self, free_plane, cmd_list):
        for plane in self.my_plane:
    ...
    def add_view(self, free_plane, cmd_list):
        if len(self.enemy_leader_plane)==2:
            # 先给我方有人机视野
            for enemy in self.enemy_plane:
                for leader in self.my_leader_plane:
    ...
    def add_view_2(self, free_plane, cmd_list):
    ...
    def update_decision(self, obs_side, cmd_list):
        ...
        if self.sim_time <= 2:
            ...
            free_plane, no_missile_plane, threat_plane_list = self.update_plane_info(cmd_list)
            # 提供视野
            self.view_support(free_plane, cmd_list)
            # 分配僚机
            self.assign_wing(free_plane, no_missile_plane)
            # 实施干扰
            self.activate_jam(cmd_list)
            # 实施攻击
            self.attack_with_jam_0(free_plane, cmd_list)
            self.attack_with_jam_1(cmd_list, threat_plane_list)
            # 保护我方有人机
            self.protect_leader(free_plane, cmd_list)
            # 无人机脱离
            self.UAV_away(free_plane, cmd_list)
            # 追击敌方有人机
            self.attack_enemy_leader(free_plane, cmd_list)
            # 扩展视野
            self.add_view(free_plane, cmd_list)
            self.add_view_2(free_plane, cmd_list)
            # 保护无人机
