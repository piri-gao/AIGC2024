...
import os
import sys
from agent.QYKZ_final.style_agent.style_agent_a import QYKZ_Agent as Agent_style_a
from agent.QYKZ_final.style_agent.style_agent_c import Agent_v2 as Agent_style_c
from agent.QYKZ_final.style_agent.style_agent_d import Agent_v3 as Agent_style_d
from agent.QYKZ_final.style_agent.style_agent_e import Agent_v4 as Agent_style_e
from agent.QYKZ_final.style_agent.style_agent_f import Agent_v5 as Agent_style_f
from agent.QYKZ_final.style_agent.style_agent_g import Agent_v6 as Agent_style_g
from agent.QYKZ_final.style_agent.style_agent_h import Agent_v7 as Agent_style_h
import re
import shutil
import random
...
class QYKZ_Agent(Agent):
    ...
    def __init__(self, name, config):
        ...
        self.style_g = Agent_style_g(name, config)
        #self.style_d = Agent_style_d(name, config)
        #self.style_e = Agent_style_e(name, config)
        self.style_f = Agent_style_f(name, config)
        self.id = 0
        self.max_id = 3
        self.alpha = 0.5
        self.epsilon = 0
    ...
    def step(self, sim_time, obs, **kwargs) -> List[dict]:
                    ...
                    if self.new_done == 1:
                        ...
                        if "双方得分相同" not in lines[-4]:
                            ...
                            if ("红方获胜" in lines[-3]) and (self.name == "red"):
                                rewards[self.id] += 1
                            if ("蓝方获胜" in lines[-3]) and (self.name == "red"):
                                rewards[self.id] -= 0.5
                            if ("红方获胜" in lines[-3]) and (self.name == "blue"):
                                rewards[self.id] -= 0.5
        ...
        if self.id == 2:
            return self.style_a.step(sim_time, obs, **kwargs)
        #if self.id == 3:
        #    return self.style_h.step(sim_time, obs, **kwargs)
        #if self.id == 4:
        #    return self.style_h.step(sim_time, obs, **kwargs)
