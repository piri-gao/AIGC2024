...
from agent.tink_AI_studio.agent_base import Plane, Missile
...
class DemoAgent(Agent):
    ...
    def update_dead_info(self):
        for plane in self.my_plane:
            ...
            for missile_id in plane.locked_missile_list.copy():
                ...
                if (tmp_missile.lost_flag and self.sim_time - tmp_missile.arrive_time > 1) or tmp_missile.loss_target:
            ...
            if plane.lost_flag:
                if len(plane.locked_missile_list)>0:
                    ...
                    for missile_id in plane.lost_missile_list:
                        ...
                        if tmp_missile.lost_flag and self.death_to_death(plane,next_plane_pos,tmp_missile,radius=100):
                            dead_flag += 1
                        if tmp_missile.loss_target==True:
                            dead_flag = -1
                    # if dead_flag==0:
                        # if self.my_score<self.enemy_score+self.win_score and self.sim_time>11*60:
                        #     if self.attack_distance-500>5200 and self.sim_time<14*60:
                        #         self.attack_distance -= 500
            ...
            for leader in self.my_leader_plane:
                ...
                if leader.Availability and need_wing:
                    if leader.wing_plane:
                        wing_plane = self.get_body_info_by_id(self.my_plane, leader.wing_plane)
                        if wing_plane.close_missile!=None and TSVector3.distance(wing_plane.pos3d,wing_plane.close_missile.pos3d)<8000:
                            continue
                        if wing_plane.ID in self.free_plane:
                            self.free_plane.remove(wing_plane.ID)
                        if len(leader.locked_missile_list) > 0:
    ...
    def init_moving(self, cmd_list):
        ...
        new_pos['Z'] = 9000
        route_list = [new_pos]
        cmd_list.append(env_cmd.make_linepatrolparam(leader_plane_1.ID, route_list, 290, 1, 6))

        new_pos = TSVector3.plus(leader_plane_2.pos3d, TSVector3.multscalar(new_dir, distance))
        new_pos['Z'] = 9000
        route_list = [new_pos]
        cmd_list.append(env_cmd.make_linepatrolparam(leader_plane_2.ID, route_list, 290, 1, 6))
    ...
    def activate_jam(self, cmd_list):
        ...
        if len(evade_list):
            for leader_id in evade_list:
                ...
                if leader.do_turn or (leader.do_jam and TSVector3.distance(leader.pos3d, leader.close_missile.pod3d)>10000):
                    continue
