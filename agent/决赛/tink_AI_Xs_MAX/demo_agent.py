...
class DemoAgent(Agent):
    ...
    def update_dead_info(self):
        ...
        for plane in self.my_plane:
            if plane.Availability:
                plane.move_speed = 0.5*plane.para["move_min_speed"]+0.5*plane.para["move_max_speed"]
                if plane.Type==2:
                    plane.move_speed = plane.para["move_min_speed"]
        ...
        # 更新导弹的制导飞机
        for missile in self.missile_list:
            if missile.Identification == self.my_plane[0].Identification:
                target_enemy = self.get_body_info_by_id(self.enemy_plane, missile.EngageTargetID)
                if target_enemy.Availability and TSVector3.distance(target_enemy.pos3d, missile.pos3d) > missile.guide_range and missile.loss_target==False and missile.lost_flag==0:
                    if missile.guide_plane != None:
                        guide_plane = self.get_body_info_by_id(self.my_plane, missile.guide_plane)
                        if len(guide_plane.locked_missile_list):
                            missile.guide_plane = None
                            guide_plane.do_guide = None
                    if len(target_enemy.followed_plane)>0:
                        if missile.guide_plane and missile.guide_plane not in target_enemy.followed_plane:
                            guide_plane = self.get_body_info_by_id(self.my_plane, missile.guide_plane)
                            missile.guide_plane =None
                            guide_plane.do_guide = None
                            continue
                        elif missile.guide_plane != None:
                            continue
                    if missile.guide_plane==None:
                        for plane in self.my_plane:
                            if plane.Availability and len(plane.locked_missile_list)==0 and plane.can_see(target_enemy, see_factor=1.0) and plane.do_guide==None:
                                if plane.Type==1 and (plane.do_jam or plane.do_turn) or plane.wing_who!=None:# 该有人机在执行干扰任务
                                    continue
                                missile.guide_plane = plane.ID
                        if missile.guide_plane!=None:
                            self.get_body_info_by_id(self.my_plane, missile.guide_plane).do_guide = target_enemy.ID
                else:
                    if missile.guide_plane != None:
                        guide_plane = self.get_body_info_by_id(self.my_plane, missile.guide_plane)
                        guide_plane.do_guide = None
        ...
        if self.sim_time <= 2:
            ...
            for my_plane in self.my_plane:
                if len(my_plane.locked_missile_list)==0 and my_plane.Availability:
                    if my_plane.do_guide:
                        cmd_list.append(env_cmd.make_followparam(my_plane.ID, my_plane.do_guide, my_plane.move_speed, my_plane.para['move_max_acc'], my_plane.para['move_max_g']))
                    if my_plane.Type==1 and (my_plane.do_jam or my_plane.do_turn) or my_plane.do_guide:# 该有人机在执行干扰任务
                        continue
            ...
            for plane_id in need_seen_plane:
                ...
                if plane.wing_plane == None or self.get_body_info_by_id(self.my_plane, plane.wing_plane).ID not in self.free_plane:
                    for see_plane_id in self.free_plane.copy():
                        see_plane = self.get_body_info_by_id(self.my_plane, see_plane_id)
                        if see_plane.can_see(follow_missile, see_factor=0.9):
                            seen_plane = see_plane
                            self.free_plane.remove(see_plane_id)
                            break
                    if seen_plane==None:
                        ...
                        for see_plane_id in self.free_plane.copy():
                            ...
                            if TSVector3.distance(see_plane.pos3d, follow_missile.pos3d)<see_plane.para['radar_range'] and turn_time<min_turn_time and turn_time<max_arrive_time:
                                seen_plane = see_plane
                                min_turn_time = turn_time
                        if seen_plane:
                            self.free_plane.remove(seen_plane.ID)
                        else:
                            continue
                else:
                            ...
                            if len(wing_plane.locked_missile_list)>0:
                                ...
                                for plane_id in no_missile_plane:
                                    ...
                                    if plane.can_see(leader_plane,see_factor=1):
                                        ...
                                        leader_plane.wing_plane = None  
                                        break
                            if leader_plane.wing_plane != None and wing_plane.ID in self.free_plane:
                                self.free_plane.remove(wing_plane.ID)
                    ...
                    if total_dir != {"X": 0, "Y": 0, "Z": 0}:
                        ...
                        if min_dis<16000:
                            ...
                            if plane.total_threat_flag == None:
                                total_threat_dir,  min_dis2 = self.synthetic_threat_vector(plane,see_dis=21955,seen=False)
                                total_threat_heading = TSVector3.calheading(total_threat_dir)
                    ...
                    for leader in self.my_leader_plane:
                        if leader.Availability and enemy.can_see(leader) and len(enemy.followed_plane)==0 and enemy.ready_missile>0:
                            ...
                            for my_plane in self.first_formation:
                                if my_plane.ID in self.free_plane:
            ...
            for plane in self.my_uav_plane:
                if plane.close_missile is None or (TSVector3.distance(plane.close_missile.pos3d,plane.pos3d)>=6000 and plane.wing_who!=None):
                    continue
                total_dir,min_dis = self.synthetic_threat_vector(plane,see_dis=25000,seen=True)
    ...
    # 计算是否逃不掉了
    def death_to_death(self, plane, new_plane_pos, missile,radius = 100):
        ...
        # if TSVector3.distance(new_missile_pos, new_plane_pos)<=radius-40 or TSVector3.distance(missile.pos3d, plane.pos3d)<=radius:
        #     return True
        # else:
        #     return False
        
        # if self.shortest_distance_between_linesegment(plane, missile)<radius or self.shortest_distance_between_linesegment(plane, missile, new_plane_pos)<radius:
        dis = self.shortest_distance_between_linesegment(plane, missile, new_plane_pos)
        if dis<radius:
            return True
        else:
            return False
    ...
    def all_death(self, plane, cmd_list):   
        ...
        for enemy in self.enemy_plane:
            if enemy.lost_flag==0 and plane.can_attack(enemy, attack_dis=21955) and TSVector3.distance(plane.pos3d, enemy.pos3d)/1000+self.sim_time+2<20*60:
                threat_plane_list.append(enemy.ID)
        if len(threat_plane_list)==0:
            for enemy in self.enemy_plane:
                if enemy.lost_flag==0 and plane.can_attack(enemy, attack_dis=plane.para['radar_range']*0.99) and TSVector3.distance(plane.pos3d, enemy.pos3d)/1000+self.sim_time+2<20*60:
                    guide_plane = None
                    for plane_ID in self.free_plane:
                        plane = self.get_body_info_by_id(self.my_plane, plane_ID)
                        if plane.can_see(enemy):
                            guide_plane = plane
                    if guide_plane != None:
                        threat_plane_list.append(enemy.ID)
        if len(threat_plane_list):
        ...
        for i in range(plane.ready_missile): # 还有导弹没利用完,保证导弹用完
            for threat_plane_id in threat_plane_list:
                ...
                if plane.ready_missile>0 and plane.ID not in threat_plane.ready_attacked:
                    ...
                    factor_fight = 1
                    print(self.sim_time,plane.Name, "开火绝地求死")
