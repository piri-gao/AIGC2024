...
class DemoDecision(Agent):
    def __init__(self, name, config):
        super(DemoDecision, self).__init__(name, config["side"])
        self._init()
    ...
    def process_observation(self, obs_side):
        ...
        for uvas_info in my_entity_infos:
            if uvas_info['ID'] != 0 and uvas_info['Availability'] > 0:
        ...
        for missile_info in missile_infos:
            if (missile_info['ID'] != 0 and missile_info['Availability'] > 0):
        ...
        if self.is_in_center(my_manned_info[0]):
            self.my_score += 1
        if len(enemy_manned_info)>0 and self.is_in_center(enemy_manned_info[0]):
            self.enemy_score += 1
        ...
        if len(self.combine_fight) == 0:
            ...
            for enemy in self.enemy_manned_info + self.enemy_uvas_infos:
                if self.missile_to_enemy_manned >= 1 and enemy['Type'] == 1 or \
                        self.missile_to_enemy_uvas - (8 - len(self.enemy_uvas_infos)) >= 1:                 # hahahhah 4->8
                    can_attack_num = 0
                    temp_list = []
                ...
                for attack_enemy in attack_enemy_list:
                    ...
                    if plane['Type'] == 1:
                        ...
                        if attack_enemy['Type'] == 1 and (self.missile_to_enemy_manned >= 1 or \
                                self.missile_to_enemy_uvas - (8 - len(self.enemy_uvas_infos)) >= 1 or plane['LeftWeapon'] <= 1): # hahahah 4->8
