...
class LiuBY_Thinker_Agent(Agent):
    """
         自定义的Demo智能体
     @Examples:
         添加使用示例
    ...
    def __init__(self, name, config):
        ...
        :param config:阵营配置信息
        """
        super(LiuBY_Thinker_Agent, self).__init__(name, config["side"])
    ...
    def enemy_has_accompany(self, enemy, myself): #单边控制 有人机仅发动 不响应
    ...
    def single_process_move(self, sim_time, cmd_list, plane):
        ...
        if len(self.my_manned_info) > 0:
            ...
            for enemy_plane in self.enemy_allplane_infos: #统计最大威胁敌机 和威胁敌机的列表  载弹且近
            ...
            #规避模块
            if len(danger_list) > 0 and not (self.safe_flag and plane['Type'] == 2): #规避
                tangent_flag = False
                dx = 0
                dy = 0
                if plane['Type'] == 1:
                    ...
                    if self.can_escape(lock_min_danger, plane,
                        ...
                        if lock_min_danger['Type'] == 3 and lock_min_time + lock_min_danger['CurTime'] - lock_min_danger['InitTime'] > 84: #不可切向规避
                            ...
                            if r < 0.0001:
                                r = 0.0001
                            dx = -dy_ * dest_distance / r  # 计算切向逃逸方向
                            dy = dx_ * dest_distance / r

                            if self.escape_last_dangerID[plane['ID']] != lock_min_danger['ID']:  # 威胁目标改变 切向方向不锁定
                                self.escape_last_dangerID[plane['ID']] = lock_min_danger['ID']
                                if lock_min_time < 15:
                                    self.escape_toward[plane['ID']] = 0

                            if self.escape_toward[plane['ID']] == 1:  # 1顺时针
                                dx = -dx
                                dy = -dy
                    ...
                    #if lock_min_distacne < danger_uvas_tangent_distance and
                    if self.can_escape(lock_min_danger, plane,
            ...
            if len(locking_list) > 0:
                ...
                enemy_v_tangent = locking_min_enemy['Speed'] * math.sin(enemy_theta)    #有方向 顺时正
                enemy_w_tangent = enemy_v_tangent / myplane_enemy_distance              #有方向 顺时正
                enemy_v_back = locking_min_enemy['Speed'] * math.cos(enemy_theta)       #有方向 远离正
                my_w_tangent = data['move_max_g'] * 9.8 / plane['Speed']                #无方向
            ...
            surround_list = danger_list + []  # 寻找可能包围自身的敌机
    ...
    def layout_point_select(self, myself):
        ...
        dest_theta = my_theta
        #if left_num == 0 and right_num != 0: #逆时针方向包抄
        if left_num < right_num:
            dest_theta = self.pi_bound(dest_theta - turn_theta)
        #elif right_num == 0 and left_num != 0: #顺时针方向包抄
    ...
    def availability(self, item): #判断对象是否合法
    ...
    def get_danger_speed(self, item): #威胁者的攻击速度
        danger_move_v = 1000
        if item['Type'] == 3 and item['CurTime'] - self.missile_init_time[item['ID']] > 30:  # 为导弹 且 发动机失效
    ...
    def get_tangent_shot_time(self, item0, item1): #粗略估计切向运动时 item0 打 item1 的时间
        distance = TSVector3.distance(item0, item1)
        start_time = 0
        enemy_turn_t = 0
        if item0['Type'] != 3:
    ...
    def get_fire_turn_time(self, myself, enemy, flag): #flag = 1 时保守估计 开火转向时间
        my_turn_theta = abs(
            self.pi_bound(self.XY2theta(enemy['X'] - myself['X'], enemy['Y'] - myself['Y']) - myself['Heading']))  # 计算偏转角
