# -*- coding: utf-8 -*-
# @Author: JYH
# @Date:   2021-10-06 12:55:59
# @Last Modified by:   Your name
# @Last Modified time: 2021-10-17 15:10:20
import random
import math
import numpy as np
...
class processor:
    ...
    def Action2cmd(self, obs_side, sim_time, actions=None):
        cmd_list = []
        if actions is None:
            ...
            for i in range(5):
                my_plane_id = self.all_id_map[i]
                my_plane = self.get_object_by_id(my_plane_id)
                if my_plane == None:
                    continue
                self.selected_module(actions[i])(sim_time, cmd_list, my_plane)
        return cmd_list

    def Mask(self, sim_time, obs_side):
        mask = np.zeros([5, 6])
        self.process_observation(obs_side, sim_time)
        for i in range(5):
            # if i == 0: print("sim_time:", sim_time)
            my_plane_id = self.all_id_map[i]
            my_plane = self.get_object_by_id(my_plane_id)
            if my_plane == None:
                mask[i, :] = 0 
                continue
            for j in range(6):
                mask[i][j] = self.test_module(j)(sim_time, my_plane)
        return mask

    def choose_action(self, pi, mask, train=True):
        if train:
            pi = pi + (mask - 1) * 1e10
            ...
            for i in range(5):
                if np.sum(mask[i, :]) == 0:
                    ...
                    pi_i = np.exp(pi[i, :]) / np.sum(np.exp(pi[i, :]))
                    # print(f"pi-{i}:", pi_i)
                    actions[i] = np.random.choice(6, p=pi_i)
            return actions
        else:
            # print("origin_pi:", pi)
            # print("mask:", mask)
            pi = pi + (mask - 1) * 1e10
            normalized_pi = np.exp(pi) / np.sum(np.exp(pi))
            # print("normalized_pi:", normalized_pi)
        ...
        # if train:
        #     actions = np.zeros(5)
        #     u = np.random.uniform(size=pi.shape) #加入噪声
        #     pi = pi - np.log(-np.log(u))
        #     pi = pi - np.expand_dims(np.min(pi, axis=-1), -1) + 1e-10 #保证>0
        #     pi = pi * mask
        # else:
        #     pi = pi + (mask-1) * 1e7
    ...
    def test_module(self, action):
        MODULE = {
            0: self.test_attack_module,
            1: self.test_avoid_module,
            2: self.test_guidance_module,
            3: self.test_escape_module,
            4: self.test_breakout_module,
            5: self.test_surround_module
        }
        return MODULE[action]

    def test_attack_module(self, sim_time, plane):
        ...
        for enemy in self.enemy_allplane_infos:
            attack_distance = self.get_attack_area(plane, enemy)
            if TSVector3.distance(enemy, plane) < attack_distance and self.is_looking(plane, enemy,
                                                                                      plane['Heading']) and (
                    plane["Type"] == 1 and plane["LeftWeapon"] > 1 or plane["Type"] == 2 and plane["LeftWeapon"] > 0):
                can_attack_list.append(enemy)
    ...
    def test_avoid_module(self, sim_time, plane):

        danger_tangent_distance = 30000  # 规避边界值
        ...
        for missile in self.enemy_missile_infos:  
            if self.availability(missile) and missile['EngageTargetID'] == plane['ID']:  
                ...
                max_attack_range = self.missile_path_distance[120] - self.missile_path_distance[
                    int(sim_time - missile['InitTime'])] + 200
                if distance > max_attack_range + \
                        data['move_max_speed'] * (120 - sim_time + missile['InitTime'] - \
                                                  self.get_escape_turn_time(plane,
                                                                            self.XY2theta(missile['X'] - plane['X'],
                                                                                          missile['Y'] - plane['Y']),
                                                                            0)):  
                    continue
        ...
        for enemy_plane in self.enemy_allplane_infos:  
            if self.availability(enemy_plane) and (enemy_plane['LeftWeapon'] > 0 and plane['Type'] == 1 or
                                                   enemy_plane['LeftWeapon'] > 2 - enemy_plane['Type'] and plane[
                                                       'Type'] == 2):  
    ...
    def test_guidance_module(self, sim_time, plane):
        ...
        if len(locking_list) == 0:
            for my_missile in self.my_missile_infos:
                if self.availability(my_missile):
                    enemy_plane = self.get_object_by_id(my_missile['EngageTargetID'])
                    if enemy_plane != None and self.availability(enemy_plane) and enemy_plane['RegimentID'] == plane[
                        'RegimentID'] and \
                            TSVector3.distance(my_missile, enemy_plane) >= free_distance:
    ...
    def test_escape_module(self, sim_time, plane):
    ...
    def test_breakout_module(self, sim_time, plane):
        danger_tangent_distance = 30000  
        danger_uvas_tangent_distance = 50000
    ...
    def test_surround_module(self, sim_time, plane):
        return 1

    def selected_module(self, action):
        MODULE = {
            0: self.attack_module,
            1: self.avoid_module,
            2: self.guidance_module,
            3: self.escape_module,
            4: self.breakout_module,
            5: self.surround_module
        }
        return MODULE[action]

    def attack_module(self, sim_time, cmd_list, plane):
        ...
        min_dist = 400000
        min_enemy = None
        for enemy in can_attack_list:
            dist = TSVector3.distance(enemy, plane)
            if dist < min_dist:
                min_dist = dist
                min_enemy = enemy

        if min_enemy is not None:
            cmd_list.append(CmdEnv.make_attackparam(plane['ID'], min_enemy["ID"], 1))

        return

    def avoid_module(self, sim_time, cmd_list, plane):
        
        dest_distance = 10000  # destination的距离
        max_bound = 145000  # destination设置的最大边界
        danger_tangent_distance = 30000  # 规避边界值
        danger_uvas_tangent_distance = 50000
        free_distance = 20000 
        ...
        lock_min_time = 2000
        for missile in self.enemy_missile_infos: 
            if self.availability(missile) and missile['EngageTargetID'] == plane['ID']:  # 被锁定
                distance = TSVector3.distance(plane, missile)
                max_attack_range = self.missile_path_distance[120] - self.missile_path_distance[
                    int(sim_time - missile['InitTime'])] + 200
                if distance > max_attack_range + \
                        data['move_max_speed'] * (120 - sim_time + missile['InitTime'] - \
                                                  self.get_escape_turn_time(plane,
                                                                            ...
                                                                            0)):  # 随便飞都打不到时
                    continue
                missile_time = self.get_tangent_shot_time(missile, plane)  # 计算时间
                danger_list.append(missile)
                if missile_time < lock_min_time:
                    lock_min_distance = distance
                    lock_min_time = missile_time
        ...
        if len(danger_list) > 0 and not (self.safe_flag and plane['Type'] == 2):
            ...
            dy = 0
            if plane['Type'] == 1:
                # if lock_min_distance < danger_tangent_distance and
                if self.can_escape(lock_min_danger, plane,
                                   self.XY2theta(plane['X'] - lock_min_danger['X'],
                                                 plane['Y'] - lock_min_danger['Y'])) is False and \
                        lock_min_danger['Type'] != 1:
                    tangent_flag = True
                    if lock_min_danger['Type'] == 3 and lock_min_time + lock_min_danger['CurTime'] - lock_min_danger[
                        'InitTime'] > 84: 
                        # print(plane['Name'], '中规避')
                        ...
                        if self.escape_toward[plane['ID']] == 1: 
                            ...
                            for danger in danger_list:
                                if danger['ID'] != lock_min_danger['ID']:
                                    if self.pi_bound(self.XY2theta(lock_min_danger['X'] - plane['X'],
                                                                   lock_min_danger['Y'] - plane['Y']) -
                                                     self.XY2theta(danger['X'] - plane['X'],
                                                                   danger['Y'] - plane['Y'])) >= 0:
                                        left_danger += 1
                                    else:
                                        right_danger += 1
                            if dx * plane['X'] + dy * plane['Y'] >= 0 and (
                                    ...
                                    or (dx * math.sin(plane['Heading']) + dy * math.cos(
                                plane['Heading']) < 0 and TSVector3.distance(lock_min_danger, plane) < 15000 \
                                        or left_danger <= right_danger and TSVector3.distance(lock_min_danger,
                                                                                              plane) >= 15000) and \
                                    (plane['X'] ** 2 + plane['Y'] ** 2) ** 0.5 < max_bound - dest_distance:
                                dx = -dx
                                dy = -dy
                ...
                # if lock_min_distance < danger_uvas_tangent_distance and
                if self.can_escape(lock_min_danger, plane,
                                   self.XY2theta(plane['X'] - lock_min_danger['X'],
                                                 plane['Y'] - lock_min_danger['Y'])) is False:
                    tangent_flag = True
                    dx_ = lock_min_danger['X'] - plane['X']
                    dy_ = lock_min_danger['Y'] - plane['Y']
                    r = (dx_ * dx_ + dy_ * dy_) ** 0.5
    ...
    def guidance_module(self, sim_time, cmd_list, plane):
        ...
        if len(locking_list) > 0:
            ...
            myplane_enemy_theta = self.XY2theta(locking_min_enemy['X'] - plane['X'],
                                                locking_min_enemy['Y'] - plane['Y'])
    ...
    def escape_module(self, sim_time, cmd_list, plane):
        ...
        for enemy_plane in self.enemy_allplane_infos:  #
            if self.availability(enemy_plane) and (enemy_plane['LeftWeapon'] > 0 and plane['Type'] == 1 or
                ...
                distance = TSVector3.distance(plane, enemy_plane)
                if distance < danger_tangent_distance and plane['Type'] == 1 or \
                        distance < danger_uvas_tangent_distance and plane['Type'] == 2:  # 进入警戒范围
                    enemy_time = self.get_tangent_shot_time(enemy_plane, plane)  # 计算时间
                    danger_list.append(enemy_plane)
                    if enemy_time < lock_min_time:
                        lock_min_distance = distance
                        lock_min_time = enemy_time
        ...
        if len(danger_list) > 0 and not (self.is_safe() and plane['Type'] == 2):  # 逃逸
            self.escape_toward[plane['ID']] = 0  # 脱离威胁 切向方向不锁定
            # if len(danger_list) == 1:  # 单威胁反向逃逸
            dx_ = plane['X'] - lock_min_danger['X']
            dy_ = plane['Y'] - lock_min_danger['Y']
            r = (dx_ * dx_ + dy_ * dy_) ** 0.5
            if r < 0.0001:
            ...
            leader_fire_route_list = [{"X": dest['X'], "Y": dest['Y'],
                                       "Z": data['area_max_alt'] - 500}, ]
            if math.sin(plane['Heading']) * (dest_x - plane['X']) + math.cos(plane['Heading']) * (
                    dest_y - plane['Y']) > 0:
                cmd_list.append(CmdEnv.make_linepatrolparam(plane['ID'], leader_fire_route_list,
                                                            data['move_max_speed'], data['move_max_acc'],
                                                            data["move_max_g"]));
    ...
    def breakout_module(self, sim_time, cmd_list, plane):
        ...
        lock_min_distance = 450000
        lock_min_time = 2000
        for missile in self.enemy_missile_infos:  # 统计最大威胁的导弹 和锁定自身的导弹列表 先按切向命中时间排序
            if self.availability(missile) and missile['EngageTargetID'] == plane['ID']:  # 被锁定
                distance = TSVector3.distance(plane, missile)
                max_attack_range = self.missile_path_distance[120] - self.missile_path_distance[
                    int(sim_time - missile['InitTime'])] + 200
        ...
        surround_list = danger_list + []  
        for enemy_surround in self.enemy_allplane_infos:
            if self.availability(enemy_surround) and self.enemy_leftweapon[enemy_surround['ID']] > 0:  # 有导弹的敌机
                distance = TSVector3.distance(plane, enemy_surround)
                if distance >= danger_enemy_distance and distance < (1 + 3 ** 0.5) * danger_enemy_distance:
                    surround_list.append(enemy_surround)
        s_dict = self.surround_dict(plane, surround_list)  # 包围圈信息
        surround_flag = 1
        if len(surround_list) < 3:
            surround_flag = 0
        else:
            for enemy_danger_ID in s_dict.keys():
                if enemy_danger_ID == plane['ID']:
                    continue
                if s_dict[plane['ID']][enemy_danger_ID]['distance'] != -1:  # 一旦有一个是包络线 （-1不是包络线）
                    surround_flag = 0
                    break
        ...
        if surround_flag == 1 and not (self.safe_flag and plane['Type'] == 2):
            ...
            if plane['Type'] == 1 or plane['Type'] == 2:
                ...
                dx = 0
                dy = 0
                for plane0 in surround_list:  # 寻找最小突围方向
    ...
    def surround_module(self, sim_time, cmd_list, plane):
    ...
    def process_observation(self, obs_side, sim_time):
        ...
        self.regiments = []
        for plane in self.enemy_allplane_infos:  # 生成团战
        ...
        for i in range(len(self.regiments)):  # 把敌方有人机的团战优先级放最后
            if self.enemy_manned_info[0] in self.regiments[i]:
                t = self.regiments[i]
                self.regiments[i] = self.regiments[-1]
                self.regiments[-1] = t
                break
        for i in range(len(self.regiments) - 2):  # 将团战从小到大排序
        ...
        for regiment_ID in range(len(self.regiments)):
            ...
            while i <= len(regiment):  # 团战分配 多分配1个
                ...
                if len(min_uvas) == 0:
                    for my_uvas in self.my_uvas_infos:
                        if my_uvas['RegimentID'] == -1 and my_uvas['LeftWeapon'] == 0:  # 无导弹
                            for enemy_plane in self.regiments[0]:
                                if TSVector3.distance(enemy_plane, my_uvas) < min_distance:
                                    min_distance = TSVector3.distance(enemy_plane, my_uvas)
                                    min_uvas = my_uvas
    ...
    def init_pos(self, cmd_list):
        ...
        init_pos_dict = {
            "close": {
                "up_mid": {
                    "red": {
                        "man": {"X": -125000, "Y": 125000 / 2, "Z": 9500},
                        0: {"X": -125000, "Y": (125000 / 2) + 10000, "Z": 9500},
                        1: {"X": -125000, "Y": (125000 / 2) + 5000, "Z": 9500},
                        2: {"X": -125000, "Y": (125000 / 2) - 5000, "Z": 9500},
                        3: {"X": -125000, "Y": (125000 / 2) - 10000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 125000 / 2, "Z": 9500},
                        0: {"X": 125000, "Y": (125000 / 2) + 10000, "Z": 9500},
                ...
                "mid": {
                    "red": {
                        "man": {"X": -125000, "Y": 0, "Z": 9500},
                        0: {"X": -125000, "Y": 10000, "Z": 9500},
                        1: {"X": -125000, "Y": 5000, "Z": 9500},
                        2: {"X": -125000, "Y": -5000, "Z": 9500},
                        3: {"X": -125000, "Y": -10000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 0, "Z": 9500},
                        0: {"X": 125000, "Y": 10000, "Z": 9500},
            ...
            "divide": {
                "wide": {
                    "red": {
                        "man": {"X": -125000, "Y": 0, "Z": 9500},
                        0: {"X": -125000, "Y": 125000, "Z": 9500},
                        1: {"X": -125000, "Y": 125000 / 2, "Z": 9500},
                        2: {"X": -125000, "Y": -125000 / 2, "Z": 9500},
                        3: {"X": -125000, "Y": -125000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 0, "Z": 9500},
                        0: {"X": 125000, "Y": 125000, "Z": 9500},
                ...
                "medium": {
                    "red": {
                        "man": {"X": -125000, "Y": 0, "Z": 9500},
                        0: {"X": -125000, "Y": 100000, "Z": 9500},
                        1: {"X": -125000, "Y": 100000 / 2, "Z": 9500},
                        2: {"X": -125000, "Y": -100000 / 2, "Z": 9500},
                        3: {"X": -125000, "Y": -100000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 0, "Z": 9500},
                        0: {"X": 125000, "Y": 100000, "Z": 9500},
                ...
                "narrow": {
                    "red": {
                        "man": {"X": -125000, "Y": 0, "Z": 9500},
                        0: {"X": -125000, "Y": 75000, "Z": 9500},
                        1: {"X": -125000, "Y": 75000 / 2, "Z": 9500},
                        2: {"X": -125000, "Y": -75000 / 2, "Z": 9500},
                        3: {"X": -125000, "Y": -75000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 0, "Z": 9500},
                        0: {"X": 125000, "Y": 75000, "Z": 9500},
            ...
            "fit": {
                "fit_up": {
                    "red": {
                        "man": {"X": -125000, "Y": 90000, "Z": 9500},
                        0: {"X": -125000, "Y": 95000, "Z": 9500},
                        1: {"X": -125000, "Y": 85000, "Z": 9500},
                        2: {"X": -125000, "Y": 5000, "Z": 9500},
                        3: {"X": -125000, "Y": -5000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 90000, "Z": 9500},
                        0: {"X": 125000, "Y": 95000, "Z": 9500},
                ...
                "fit_mid": {
                    "red": {
                        "man": {"X": -125000, "Y": 0, "Z": 9500},
                        0: {"X": -125000, "Y": 5000, "Z": 9500},
                        1: {"X": -125000, "Y": -5000, "Z": 9500},
                        2: {"X": -125000, "Y": 55000, "Z": 9500},
                        3: {"X": -125000, "Y": -55000, "Z": 9500}
                    },
                    "blue": {
                        "man": {"X": 125000, "Y": 0, "Z": 9500},
                        0: {"X": 125000, "Y": 5000, "Z": 9500},
                ...
                "fit_down": {
                    "red": {
                        "man": {"X": -125000, "Y": -90000, "Z": 9500},
                        0: {"X": -125000, "Y": -85000, "Z": 9500},
                        1: {"X": -125000, "Y": -95000, "Z": 9500},
                        2: {"X": -125000, "Y": 5000, "Z": 9500},
                        3: {"X": -125000, "Y": -5000, "Z": 9500}
                    },
                    "blue": {
        ...
        init_area = random.choice(["close", "divide", "fit"])
        if init_area == "close":
            init_area = random.choice(["up_mid", "mid", "mid_down"])
            for leader in self.my_manned_info:  #
                # CmdEnv.make_entityinitinfo 指令，可以将 飞机实体置于指定位置点，参数依次为 实体ID，X坐标，Y坐标，Z坐标，初始速度，初始朝向
                cmd_list.append(CmdEnv.make_entityinitinfo(leader['ID'],
                                                           init_pos_dict["close"][init_area][self.name]["man"][
                                                               'X'],
                                                           init_pos_dict["close"][init_area][self.name]["man"][
                                                               'Y'],
                                                           init_pos_dict["close"][init_area][self.name]["man"][
                                                               'Z'], 400, 45))
            for i, sub in enumerate(self.my_uvas_infos):
                cmd_list.append(
                    CmdEnv.make_entityinitinfo(sub['ID'], init_pos_dict["close"][init_area][self.name][i]['X'],
                                               init_pos_dict["close"][init_area][self.name][i]['Y'],
                                               init_pos_dict["close"][init_area][self.name][i]['Z'], 300, 45))
        elif init_area == "divide":
            init_area = random.choice(["wide", "medium", "narrow"])
            for leader in self.my_manned_info:
                # CmdEnv.make_entityinitinfo 指令，可以将 飞机实体置于指定位置点，参数依次为 实体ID，X坐标，Y坐标，Z坐标，初始速度，初始朝向
                cmd_list.append(CmdEnv.make_entityinitinfo(leader['ID'],
                                                           init_pos_dict["divide"][init_area][self.name]["man"][
                                                               'X'],
                                                           init_pos_dict["divide"][init_area][self.name]["man"][
                                                               'Y'],
                                                           init_pos_dict["divide"][init_area][self.name]["man"][
            ...
            for i, sub in enumerate(self.my_uvas_infos):
                cmd_list.append(
                    CmdEnv.make_entityinitinfo(sub['ID'], init_pos_dict["divide"][init_area][self.name][i]['X'],
                                               init_pos_dict["divide"][init_area][self.name][i]['Y'],
                                               init_pos_dict["divide"][init_area][self.name][i]['Z'], 300, 45))
        else:
            init_area = random.choice(["fit_up", "fit_mid", "fit_down"])
            for leader in self.my_manned_info:
                # CmdEnv.make_entityinitinfo 指令，可以将 飞机实体置于指定位置点，参数依次为 实体ID，X坐标，Y坐标，Z坐标，初始速度，初始朝向
                cmd_list.append(CmdEnv.make_entityinitinfo(leader['ID'],
                                                           init_pos_dict["fit"][init_area][self.name]["man"]['X'],
                                                           init_pos_dict["fit"][init_area][self.name]["man"]['Y'],
                                                           init_pos_dict["fit"][init_area][self.name]["man"]['Z'],
                                                           400, 45))
            for i, sub in enumerate(self.my_uvas_infos):
                cmd_list.append(
                    CmdEnv.make_entityinitinfo(sub['ID'], init_pos_dict["fit"][init_area][self.name][i]['X'],
                                               init_pos_dict["fit"][init_area][self.name][i]['Y'],
                                               init_pos_dict["fit"][init_area][self.name][i]['Z'], 300, 45))
    ...
    def layout_point_select(self, myself):
        ...
        if TSVector3.distance(select_plane, myself) < 0.0001:  # 防safe时除0
            return {'X': myself['X'] + math.sin(myself['Heading']) * 1000,
                    'Y': myself['Y'] + math.cos(myself['Heading']) * 1000}
        ...
        if myself['LeftWeapon'] == 0 or myself['Type'] == 1:
            ...
            for plane in self.my_allplane_infos:
                ...
                if self.availability(plane) and plane['ID'] != myself['ID'] and plane['RegimentID'] == myself[
                    'RegimentID'] and plane['LeftWeapon'] > 2 - plane['Type']:  # 我方载弹飞机包夹敌方
        ...
        closest_plane = {}  # 最近的有威胁的飞机
        for enemy_plane in self.enemy_allplane_infos:
            if enemy_plane['LeftWeapon'] > 2 - enemy_plane['Type'] and myself['Type'] == 2 or \
                    enemy_plane['LeftWeapon'] > 0 and myself['Type'] == 1:  # TODO:ememy_plane
        ...
        if len(closest_plane) != 0 and my_closest_distance <= data0[
            'launch_range'] and not self.safe_flag or self.safe_flag:
            if (myself['LeftWeapon'] == 0 or self.is_regiment_win(myself['RegimentID']) is False or myself[
                'Type'] == 1) and not self.safe_flag:
                looking_distance = 58000
                if myself['Type'] == 1:
                    looking_distance = 50000
                return self.get_KeepDistance_move_XY(myself, closest_plane, looking_distance)
            elif (myself['Type'] == 1 or myself['Type'] == 2 and not self.is_looking(myself, closest_plane, myself[
                'Heading']) and my_closest_distance < 10000) and self.safe_flag:
                looking_distance = 78000  # TODO: 跑这么远？
                dest = self.get_KeepDistance_move_XY(myself, closest_plane, looking_distance)
    ...
    def layout_plane_select(self, myself):  # 选择团战中离有人机最近的飞机
        regimentID = myself['RegimentID']
        min_distance = 450000
        select_plane = {}
        for enemy_plane in self.regiments[regimentID]:
            distance = TSVector3.distance(enemy_plane, self.my_manned_info[0])
            ...
            for plane in self.my_allplane_infos:
                for enemy_plane in self.enemy_allplane_infos:
                    if TSVector3.distance(enemy_plane, my_leader) < 15000 and len(
                        ...
                        if plane['Type'] == 1 and plane['LeftWeapon'] > 1 and not self.is_attacking(plane,
                                                                                                    enemy_plane) and self.is_looking(
                                plane, enemy_plane, plane['Heading']):
                            ...
                            for my_uvas in self.my_uvas_infos:
                                if my_uvas['LeftWeapon'] > 0 and not self.is_in_cold(my_uvas, enemy_plane) and \
                                        TSVector3.distance(my_uvas, enemy_plane) < TSVector3.distance(plane,
                                                                                                      enemy_plane):
                                    min_flag = False
                            if min_flag:  # 立刻救援
                                can_attack_dict[enemy_plane['ID']].append(plane['ID'])
        ...
        for enemy_plane in self.enemy_uvas_infos + self.enemy_manned_info:
            ...
            for my_plane in self.my_allplane_infos:
                if self.is_looking(enemy_plane, my_plane, enemy_plane['Heading']) and self.is_guidanced(my_plane):
                    ...
                    break
            if enemy_looking_flag and len(enemy_plane['LockedMissileID']) + self.curtime_has_attack[
                enemy_plane['ID']] == 0:
            ...
            if enemy_plane in attacked_list_1:
                ...
                for my_plane in self.my_allplane_infos:
                    ...
                    if not curtime_attack_myplane[my_plane['ID']] and not self.is_attacking(my_plane, enemy_plane) and \
                            (my_plane['LeftWeapon'] > 0 and TSVector3.distance(enemy_plane, my_plane) < data[
                                'launch_range'] and my_plane['Type'] == 2 or \
                             my_plane['LeftWeapon'] > 1 and TSVector3.distance(enemy_plane, my_plane) < data[
                                 'launch_range'] and my_plane['Type'] == 1 and \
                ...
                for i in range(rest):
                    ...
                    for my_plane in self.my_allplane_infos:
                        data = self.get_move_data(my_plane)
                        if (my_plane['LeftWeapon'] > 1 and my_plane['Type'] == 1 or my_plane['LeftWeapon'] > 0 and
                            my_plane['Type'] == 2) and \
                                not self.is_attacking(my_plane, enemy_plane) and not curtime_attack_myplane[
                            my_plane['ID']] and \
                                TSVector3.distance(enemy_plane, my_plane) < self.get_attack_area(my_plane, enemy_plane):
                            distance = TSVector3.distance(my_plane, enemy_plane)
                            if my_plane['LeftWeapon'] > max_leftweapon or my_plane[
                                'LeftWeapon'] == max_leftweapon and distance < min_distance:
                ...
                if not self.is_in_cold(my_plane, enemy_leader) and my_plane['LeftWeapon'] > 2 - my_plane['Type'] and \
                    ...
                    if not fire_flag and TSVector3.distance(my_plane, enemy_leader) < 20000:  # combine开火
                        for trigger_missile in self.my_missile_infos:
                            ...
                            if d_theta > math.pi / 3 and abs(
                                    self.get_tangent_shot_time(my_plane, enemy_leader) - self.get_tangent_shot_time(
                                            trigger_missile, enemy_leader)) < 10:
                                fire_flag = True
                                fire_type = 2
                                break
                    if fire_flag:
                        ...
                        # print(my_plane['Name'], fire_type)

        return can_attack_dict

    def is_safe(self):
        if len(self.enemy_uvas_infos) == 0 and self.enemy_manned_info[0]['LeftWeapon'] <= 1 and len(
                self.enemy_missile_infos) == 0:
            return True
        return False

    def get_KeepDistance_move_XY(self, myself, danger, distance):
        bound = 145000
    ...
    def is_regiment_win(self, regiment_ID):
        my_win = 0
        for my_plane in self.my_allplane_infos:
    ...
    def is_in_cold(self, item0, item1):  # 是否在冷却时间内
        cold_time = 10
        min_time = 1200
        for missile in self.my_missile_infos + self.enemy_missile_infos:
            if missile['LauncherID'] == item0['ID'] and missile['EngageTargetID'] == item1['ID'] and item0['CurTime'] - \
                    missile['InitTime'] < min_time:
    ...
    def can_escape(self, item0, item1, theta):
        ...
        if item0['Type'] == 3:
            ...
            missile_distance = ((TSVector3.distance(item0, item1) * math.sin(
                missile_plane_theta) + plane_distance * math.sin(theta)) ** 2 + \
                                (TSVector3.distance(item0, item1) * math.cos(
                                    missile_plane_theta) + plane_distance * math.cos(theta)) ** 2) ** 0.5
            ...
            for i in range(int(start_time), 122):
                ...
                if self.missile_path_distance[i] - self.missile_path_distance[int(start_time)] + 200 >= distance:
                    ...
                    break
            if shot_time > self.get_escape_turn_time(item1,
                                                     self.XY2theta(item0['X'] - item1['X'], item0['Y'] - item1['Y']),
                                                     0):
                return True
            return False
    ...
    def get_danger_shot_time(self, item0, item1, theta):  # 粗略估计 item0 打 item1 的时间 theta为逃逸方向
        data = self.get_move_data(item1)
        my_turn_t = self.get_escape_turn_time(item1, theta, 0)
        start_time = 0
        enemy_turn_t = 0
        if item0['Type'] != 3:
            enemy_turn_t = self.get_fire_turn_time(item0, item1, 1)  # 计算item0打item1的偏转时间
        else:
            start_time = item0['CurTime'] - item0['InitTime']
        for i in range(int(start_time), 121):
    ...
    def get_escape_turn_time(self, myself, theta, flag):  # flag = 1 时保守估计 逃跑转向时间
    ...
    def get_fire_turn_time(self, myself, enemy, flag):  # flag = 1 时保守估计 开火转向时间
        my_turn_theta = abs(
            self.pi_bound(
                self.XY2theta(enemy['X'] - myself['X'], enemy['Y'] - myself['Y']) - myself['Heading']))  # 计算偏转角
        look_theta = math.pi / 3  # 计算探测角
        if myself['Type'] == 2:
            look_theta /= 2
    ...
    def dest_bound(self, plane, dest_x, dest_y, max_bound, dest_distance):  # 将目标限定到圆形区域
    ...
    def is_out_of_map(self, x, y):  # 判定是否超出地图
    ...
    def is_looking(self, item0, item1, item0_toward_theta):  # 判定item0是否正监测着item1
    ...
    def is_attacking(self, item0, item1):  # 判定item0是否正在攻击item1
        for missile in self.enemy_missile_infos + self.my_missile_infos:
            if self.availability(missile) and missile['LauncherID'] == item0['ID'] and missile['EngageTargetID'] == \
                    item1['ID']:
                return True
        return False

    def get_attack_area(self, myself, enemy):  # 返回当前角度开火范围
        data0 = self.get_move_data(myself)
        ...
        # print('attack_area: ', myself['Name'], enemy['Name'], max_area)
        return max_area
    ...
    def get_missile_data(self):
        missile_path_distance = [0, 542.2063609607588, 1182.480494005152, 1920.822152703217, 2757.231069395307,
                                 ...
                                 91838.29318928291 + 525.8610703383404]
