...
class DemoDecision():
                ...
                if attack_plane is not None:
                    ...
                    if can_attack_now:
                        if attack_plane.Type == 1:
                            ...
                            factor_fight = distance/attack_plane.performance()["launch_range"]
    ...
    def get_threat_target_list(self):
        ...
        if enemy_leader:
            ...
            threat_plane_dict = threat_plane_list
            # threat_plane_dict = []
        ...
        for hit_enemy in self.hit_enemy_list:
            leader_hit = False
            # 敌有人机可以打两发
            if hit_enemy[0].num_locked_missile < 2 and hit_enemy[0].Type == 2:
                leader_hit = True
            elif hit_enemy[0].num_locked_missile < 3 and hit_enemy[0].Type == 1:
                leader_hit = True
            for threat_plane in threat_plane_list:
